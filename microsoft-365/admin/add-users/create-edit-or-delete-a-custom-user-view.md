...
- Microsoft から Microsoft 365 を購入しなかった場合、課金管理者は標準ビューリストに表示されません。 詳細については、「[管理者ロールを割り当てる](assign-admin-roles.md)」を参照してください。 
...
> [!NOTE]
> **Exchange メールボックス フィルターを持つライセンスのないユーザー** は、次の場合に機能します。
1. メールボックスは最近、共有からユーザー **に** 変換 **され** 、ライセンスはありません。
2. メールボックスは最近 Microsoft 365 に移行されましたが、ライセンスが割り当てられていない。
