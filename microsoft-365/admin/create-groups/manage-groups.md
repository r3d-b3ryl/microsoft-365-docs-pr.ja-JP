---
title: 管理センターで Office 365 グループを管理する
ms.reviewer: arvaradh
f1.keywords:
- NOCSH
ms.author: mikeplum
author: MikePlumleyMSFT
manager: pamgreen
audience: Admin
ms.topic: get-started-article
ms.service: o365-administration
localization_priority: Normal
ms.collection:
- M365-subscription-management
- Adm_O365
- Adm_TOC
search.appverid:
- BCS160
- MET150
- MOE150
ms.assetid: 74a1ef8b-3844-4d08-9980-9f8f7a36000f
description: グループメンバーの追加、電子メールアドレス、グループ名、説明の編集、グループの動作のカスタマイズなど、Office 365 グループを管理する方法について説明します。
ms.openlocfilehash: ed965cab10899624d85763b3f91f20b2df508c0e
ms.sourcegitcommit: ca2b58ef8f5be24f09e73620b74a1ffcf2d4c290
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 02/24/2020
ms.locfileid: "42241344"
---
# <a name="manage-an-office-365-group-in-the-microsoft-365-admin-center"></a>Microsoft 365 管理センターで Office 365 グループを管理する

[Office 365 グループを作成](create-groups.md)し、グループメンバーを追加したら、グループを構成できます。 グループ名または説明を編集したり、所有者またはメンバーを管理したり、外部の送信者がグループに電子メールを送信できるかどうか、およびグループスレッドのコピーをメンバーに送信するかどうかを指定したりできます。 

## <a name="edit-a-group-name"></a>グループ名を編集する

::: moniker range="o365-worldwide"

1. 管理センターで、[**グループ** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=2052855" target="_blank">グループ</a>] ページに移動し、変更するグループのチェックボックスをオンにします。

2. **[名前の編集]** を選択します。

3. [**詳細の編集**] ウィンドウで、グループの新しい名前を入力し、[**保存**] を選択します。

::: moniker-end

::: moniker range="o365-germany"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、変更するグループの名前を選択します。

2. [**名前**] の横にある [**編集**] を選択します。

3. [**詳細の編集**] ウィンドウで、グループの新しい名前を入力し、[**保存**] を選択します。

::: moniker-end

::: moniker range="o365-21vianet"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、変更するグループの名前を選択します。

2. [**名前**] の横にある [**編集**] を選択します。

3. [**詳細の編集**] ウィンドウで、グループの新しい名前を入力し、[**保存**] を選択します。

::: moniker-end


## <a name="edit-a-group-description"></a>グループの説明を編集する

::: moniker range="o365-worldwide"

1. 管理センターで、[**グループ** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=2052855" target="_blank">グループ</a>] ページに移動し、説明を変更するグループのチェックボックスをオンにします。

2. [**編集の説明**] を選択します。

3. [**詳細の編集**] ウィンドウで、グループの新しい説明を入力し、[**保存**] を選択します。

::: moniker-end

::: moniker range="o365-germany"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、説明を変更するグループの名前を選択します。

2. [**説明**] の横にある [**編集**] を選択します。

3. [**詳細の編集**] ウィンドウで、グループの新しい説明を入力し、[**保存**] を選択します。

::: moniker-end

::: moniker range="o365-21vianet"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、説明を変更するグループの名前を選択します。

2. [**説明**] の横にある [**編集**] を選択します。

3. [**詳細の編集**] ウィンドウで、グループの新しい説明を入力し、[**保存**] を選択します。

::: moniker-end


## <a name="manage-group-owners"></a>グループの所有者を管理する

::: moniker range="o365-worldwide"

1. 管理センターで、[**グループ**] \> [<a href="https://go.microsoft.com/fwlink/p/?linkid=2052855" target="_blank">グループ</a>] ページの順に移動します。

2. 管理するグループの名前を選択し、[設定] ウィンドウを開きます。

3. [**メンバー** ] タブの [**所有者**] で、[すべて表示] を選択し、[**所有者の管理**] を選択して、次のいずれかの操作を行います。

    - 所有者を追加するには、[所有者の**追加**] を選択し、所有者を検索または選択して、[**保存**] を選択します。 
    - 所有者を削除するには、グループの所有者を検索または選択し、その人の名前の横にある**X**を選択して、[**保存**] を選択します。 

::: moniker-end

::: moniker range="o365-germany"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、所有者を変更するグループの名前を選択します。

2. [**所有者**] の横にある [**編集**] を選択し、次のいずれかの操作を行います。

    - 所有者を追加するには、[所有者の**追加**] を選択し、所有者を検索または選択して、[**保存**] を選択します。 
    - 所有者を削除するには、グループの所有者を検索または選択し、その人の名前の横にある**X**を選択して、[**保存**] を選択します。 

::: moniker-end

::: moniker range="o365-21vianet"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、所有者を変更するグループの名前を選択します。

2. [**所有者**] の横にある [**編集**] を選択し、次のいずれかの操作を行います。

    - 所有者を追加するには、[所有者の**追加**] を選択し、所有者を検索または選択して、[**保存**] を選択します。 
    - 所有者を削除するには、グループの所有者を検索または選択し、その人の名前の横にある**X**を選択して、[**保存**] を選択します。 

::: moniker-end

## <a name="manage-group-members"></a>グループメンバーを管理する

::: moniker range="o365-worldwide"

1. 管理センターで、[**グループ**] \> [<a href="https://go.microsoft.com/fwlink/p/?linkid=2052855" target="_blank">グループ</a>] ページの順に移動します。

2. メンバーを管理するグループの名前を選択し、[設定] ウィンドウを開きます。

3. [**メンバー** ] タブの [**メンバー**] で、[**すべて表示**] を選択して、[メンバーの管理] を選択し、次のいずれかの操作を行います。

    - メンバーを追加するには、[**メンバーの追加**] を選択し、メンバーを検索または選択して、[**保存**] を選択します。 
    - メンバーを削除するには、グループメンバーを検索または選択し、その人の名前の横にある**X**を選択して、[**保存**] を選択します。 

::: moniker-end

::: moniker range="o365-germany"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、変更するメンバーを含むグループの名前を選択します。

2. [**メンバー**] の横にある [**編集**] を選択し、次のいずれかの操作を行います。

    - メンバーを追加するには、[**メンバーの追加**] を選択し、メンバーを検索または選択して、[**保存**] を選択します。 
    - メンバーを削除するには、グループメンバーを検索または選択し、その人の名前の横にある**X**を選択して、[**保存**] を選択します。 

::: moniker-end

::: moniker range="o365-21vianet"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、変更するメンバーを含むグループの名前を選択します。

2. [**メンバー**] の横にある [**編集**] を選択し、次のいずれかの操作を行います。

    - メンバーを追加するには、[**メンバーの追加**] を選択し、メンバーを検索または選択して、[**保存**] を選択します。 
    - メンバーを削除するには、グループメンバーを検索または選択し、その人の名前の横にある**X**を選択して、[**保存**] を選択します。  

::: moniker-end

## <a name="send-copies-of-conversations-to-group-members-inboxes"></a>会話のコピーをグループメンバーの受信トレイに送信する
  
管理センターを使用してグループを作成する場合、既定では、受信トレイに送信されるグループメールと会議出席依頼のコピーは取得されません。 会話や会議を表示するには、グループに移動する必要があります。 この設定は、管理センターで変更できます。

この設定をオンにすると、グループのメンバーは、Outlook 受信トレイに送信されたグループメールと会議出席依頼のコピーを取得します。 各メンバーはメールのコピーを閲覧、削除することができますが、他のメンバーに影響を与えることはありません。 グループの受信トレイに、メールのコピーは引き続き存在します。

グループメンバーは、Outlook でグループのフォローを停止することを選択することで、これらのメールを受信しないようにすることができます。

::: moniker range="o365-worldwide"

1. 管理センターで、[**グループ** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=2052855" target="_blank">グループ</a>] ページに移動し、変更するグループの名前を選択します。 

2. [**設定**] タブで、メンバーがグループメッセージおよび予定表アイテムのコピーを自分の受信トレイに受信するようにする場合は、[グループ**の会話とイベントのコピーをグループのメンバーに送信**する] を選択します。

3. [**保存**] を選択します。

::: moniker-end

::: moniker range="o365-germany"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、変更するグループを選択します。

2. [**名前**] の横にある [**編集**] を選択します。

3. メンバーが自分の受信トレイにグループメッセージと予定表アイテムのコピーを受信するようにする**には、** グループ**の会話とイベントのコピーをグループメンバーの受信トレイに送信**します。

4. [**保存**] を選択します。

::: moniker-end

::: moniker range="o365-21vianet"

1. 管理センターで、[**グループ** \> **グループ**] ページに移動し、変更するグループを選択します。

2. [**名前**] の横にある [**編集**] を選択します。

3. メンバーが自分の受信トレイにグループメッセージと予定表アイテムのコピーを受信するようにする**には、** グループ**の会話とイベントのコピーをグループメンバーの受信トレイに送信**します。

4. [**保存**] を選択します。

::: moniker-end

## <a name="let-people-outside-the-organization-email-the-group"></a>組織外のユーザーがグループに電子メールを送信できるようにする

このオプションは、info@contoso.com のような会社の電子メールアドレスを使用する場合に適しています。
 
::: moniker range="o365-worldwide"

1. 新しいグループが表示されるように、管理センターページを更新します。

2. [管理センターグループ] リストで、変更するグループの名前を選択し、[**設定**] タブで [**外部の送信者にメールを送信することを許可する**] を選択します。
    
4. [**保存**] を選択します。

::: moniker-end

::: moniker range="o365-germany"

1. 新しいグループが表示されるように、管理センターページを更新します。
    
2. [管理センターグループ] リストで、変更するグループを選択し、[**名前**] の横にある [**編集**] を選択します。 
    
3. [ **組織外のユーザーにグループへのメールを許可します**] トグルを **オン**にします。
    
4. [**保存**] を選択します。

::: moniker-end

::: moniker range="o365-21vianet"

1. 新しいグループが表示されるように、管理センターページを更新します。
    
2. [管理センターグループ] リストで、変更するグループを選択し、[**名前**] の横にある [**編集**] を選択します。 
    
3. [ **組織外のユーザーにグループへのメールを許可します**] トグルを **オン**にします。
    
4. [**保存**] を選択します。

::: moniker-end

## <a name="related-articles"></a>関連記事

[Office 365 グループを作成する](create-groups.md)

[Office 365 グループへのゲストアクセスを管理する](https://support.office.com/article/7c713d74-a144-4eab-92e7-d50df526ff96.aspx)

[Office 365 グループを作成するときに使用するドメインを選択する](choose-domain-to-create-groups.md)

[Office 365 グループの代理人として送信または送信することをメンバーに許可する](allow-members-to-send-as-or-send-on-behalf-of-group.md)

[配布リストを Office 365 グループにアップグレードする](../manage/upgrade-distribution-lists.md)

[PowerShell で Office 365 グループを管理する](https://support.office.com/article/aeb669aa-1770-4537-9de2-a82ac11b0540)
