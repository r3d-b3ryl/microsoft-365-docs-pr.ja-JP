...
1. [レコード **の追加]** を選択し **、[TXT] セクションを選択** します。
...
1. [DNS レコードの追加] ページで、新しいレコードのボックスに、次の表の値を入力またはコピーして貼り付けます。 
    ...
    |:-----|:-----|:-----|
  |**ホスト名** <br/> |**値** <br/> |
|(このフィールドは空白のままにする)  <br/> |MS=ms *XXXXXXXX*  <br/> 注: これは例です。 この表から **[宛先またはポイント先のアドレス]** の値を指定してください。 [確認する方法](../get-help-with-domains/information-for-dns-records.md)          |
...
1. [レコード **の追加]** を選択し **、[MX] セクションを選択** します。
...
8. **[保存]** を選択します。<br/>(下へスクロールしなければならないことがあります。)<br/>

6. 既にリストされている MX レコードがある場合は、[レコードの削除] を選択して、各レコード **を削除します**。
...
1. 免責事項の [ **I am aware**] チェック ボックスをオンにします。<br/>
...
1. [ **レコードの追加]** を選択し **、[SPF (TXT)] セクションを選択** します。
...
1. 上記の最初の CNAME レコードの手順を使用して、別のサブドメイン (Lyncdiscover) を作成します。
...
1. [レコード **の追加]** を選択し **、[SRV] セクションを選択** します。
...
> [!IMPORTANT]
> 他の CNAME レコードに使用した手順に従い、次の表の値を指定します。 
