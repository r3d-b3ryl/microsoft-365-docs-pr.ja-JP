...
3. **[+ エイリアスを追加]** を選択し、ユーザーの新しいエイリアスを入力します。   
   ...
   > [!IMPORTANT]
   >  "**このユーザーは、ローカルの Active Directory と同期されています‎。一部の詳細は、ローカルの Active Directory を介してのみ編集できます。**" というエラー メッセージが表示される場合、Azure Directory が同期済みユーザーの属性に対して権限を持っているため、オンプレミスの Active Directory で属性を変更する必要があります。
