...
メッセージ センターのメールの登録を解除するには、この記事の「 [メッセージ センターからのメール](#unsubscribe-from-message-center-emails) の購読を解除する」を参照してください。
...
|タグ| メッセージをフィルター処理するには、[タグ] のドロップダウンから選択できます。 <br> <p> **データのプライバシー**: データのプライバシーに関する通知 (グローバル管理者とメッセージ センターのプライバシー 閲覧者の役割に限定)。 <p> **メジャー更新** プログラム: 少なくとも 30 日前に通信された変更 (メジャー [更新プログラム](#major-updates))。 <p> **退職**: サービスまたは機能の削除。 <p> **新機能:** 新機能またはサービス。 <p> **機能の更新**: 既存の機能に更新します。 <p> **管理者への影響**: UI の変更、ワークフローの変更、利用可能な制御、特定/潜在的なアクションなど、変更が管理者に明らかに影響を与えた場合。 <p> **ユーザーの影響**: サービスへの変更がユーザーに明らかに影響を与える場合 - UI の変更とワークフローの変更。 <p> **更新されたメッセージ**: メッセージが更新された場合。|
...
[新機能 (Office) に表示される機能を](../manage/show-hide-new-features.md)管理する
