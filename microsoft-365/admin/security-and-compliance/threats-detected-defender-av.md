...
脅威の保護を提供するために、Microsoft Defender ウイルス対策方法を使用します。 これらの方法には、クラウドによる保護、リアルタイム保護、専用の保護更新プログラムが含まれます。

- クラウドによって提供される保護は、新しい脅威や新たな脅威のほぼ瞬時の検出とブロックを提供するのに役立ちます。
- 常時スキャンでは、ファイルおよびプロセス動作の監視などの手法 (リアルタイム保護とも呼ばれる) *を使用します*。
...
- [次世代のセキュリティ保護Windows 10](/windows/security/threat-protection/microsoft-defender-antivirus/microsoft-defender-antivirus-in-windows-10)
...
無効モードの場合でも、ユーザーと顧客はスケジュールされたスキャンまたはオンデマンド スキャンで Microsoft Defender ウイルス対策 を使用して脅威を特定できます。ただし、Microsoft Defender ウイルス対策は使用できなくなります。
...
> [!TIP]
> - アプリを使用している場合は、Microsoft 365ウイルス対策ソリューションMicrosoft Defender ウイルス対策使用を検討してください。 統合は、より良い保護を提供できます。 「[Better together: Microsoft Defender ウイルス対策 と Office 365」を参照してください](/windows/security/threat-protection/microsoft-defender-antivirus/office-365-microsoft-defender-antivirus)。
...
- ユーザーは、[ユーザーに通知をWindows](https://support.microsoft.com/windows/8942c744-6198-fe56-4639-34320cf9444e)。 
- 検出は、[保護の履歴] [ページWindows セキュリティアプリ](/windows/security/threat-protection/windows-defender-security-center/windows-defender-security-center)の一 **覧に表示** されます。  
- Windows 10 デバイスをセキュリティで保護し、[Intune](/mem/intune/enrollment/windows-enrollment-methods) に登録し、組織に 800 以下のデバイスが登録されている場合は、[脅威とウイルス対策] ページの  [Microsoft 365 管理センター](../setup/secure-win-10-pcs.md) に脅威の検出と分析情報が表示され <a href="https://go.microsoft.com/fwlink/p/?linkid=2024339" target="_blank"></a>、Microsoft Defender ウイルス対策 からアクセス **できます。**[ホーム] **ページ (** > または [**HealthThreats**] を選択してナビゲーション ウィンドウから&します。

    組織に Intune に登録されているデバイスが 800 台を超える場合は、[脅威とウイルス対策] ページではなく[、Microsoft エンドポイント マネージャー](/mem/endpoint-manager-overview) からの脅威の検出と分析情報を表示するように求められます。
 
    > [!NOTE]
    > [**Microsoft Defender ウイルス対策****および脅威と** ウイルス対策] ページは段階的に展開され、すぐにアクセスできない場合があります。
...
[デバイスWindowsセキュリティで保護](/misc/m365bp-secure-windows-devices)する (記事)\
