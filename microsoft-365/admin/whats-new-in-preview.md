...
## <a name="january-2020---happy-new-year"></a>2020年1月-新年おめでとう。

> [!NOTE]
> YouTube に関する Microsoft 365 ビデオシリーズの[新機能](https://go.microsoft.com/fwlink/p/?linkid=2118096)についてご存知ですか? ユーザーに展開された最新の機能が強調されています。 毎月、「[ビデオ、トレーニング、およびドキュメント](#videos-training-and-docs)」の最後のエピソードへのリンクを開始します。 <br> <br> January's エピソード: [2020 年1月-Microsoft での新機能 365](https://go.microsoft.com/fwlink/p/?linkid=2118096)
...
詳細については、「」を参照してください。 [クレジット カードまたは銀行口座から請求書に変更する](../commerce/billing-and-payments/change-payment-method.md#change-from-credit-card-or-bank-account-to-invoice)
...
> [!IMPORTANT]
> 通常の URL を使用して SharePoint 管理センターにアクセスできるようになります。https://admin.microsoft.com/SharePoint
...
### <a name="featured-feedback-fix--the-option-to-convert-the-deleted-users-mailbox-to-a-shared-mailbox-is-back"></a>特集フィードバックの修正–削除されたユーザーのメールボックスを共有メールボックスに変換するオプションがバックアップされます。
...
- 今月の**請求**変更には、bonanza があります。
    - 既存のクレジットカードを削除して再度追加することなく、CVV を更新できるようになりました。 CVV を更新するには、[ **** > **支払い方法**(請求書) に進みます。
...
- また、いくつかの小さな機能強化とバグ修正も行いました。
  ...
  - [インベントリの詳細] ページでは、通知の拡張機能が追加されています。ここでは、作業を行うことができる実際のページにリンクします。また、インベントリの詳細カードには、さらに多くの操作があります。 請求書**** > **の請求書** > の**詳細を表示**します。

## <a name="january-2019---happy-new-year"></a>2019年1月-新年おめでとう。
...
- **サービス & アドイン**-10 月に、新しいデザインスタイルで**設定** \> **サービス & アドイン**を更新しましたが、すべての設定ページは従来のスタイルのままです。 今月は、実際の設定を新しいデザインに更新し始めています。 **予約**をチェックアウトして新しい外観を表示します。
- **ユーザー管理**-ユーザー名、エイリアス、プライマリ電子メールアドレスを管理します。
- ライブラリに2つのホームカードを追加しました。 ライブラリの詳細については、「」を参照してください。
  - **Intune デバイスの登録**
  - **ドメイン**
- **推奨事項**-管理センターに新しいアドバイスポップアップが表示される場合があります。 お客様の組織にとってメリットがあると思われる場合にのみ、推奨事項が表示されます。 ただし、カードライブラリから追加できる推奨事項が表示されるまで待機しないでください。
  - **機密情報を保護**する-機密情報を含むアイテムを組織外のユーザーと共有する場合に、データ損失防止 (DLP) ポリシーを使用して検出するようにすることをお勧めします。

## <a name="november-2018"></a>2018 年 11 月

- 検索ボックスが戻る 少しだけ移動しました。 これはホームページの上部にありますが、[**カードの追加**] ボタンの近くのページの右側にあります。 <br> ![検索ボックスがホームページの右上にあることを説明する画面キャプチャ。](media/M365PreviewSearchBox.png)

- ダッシュボードのカードを追加しました。 管理センターに移動し、[**カードの追加**] を選択して新しいカードを表示するだけです。
  - **Office 365 のアクティブユーザーレポート**-購入した office 365 サービスの使用状況を監視します。
  - **Office 365 software** -Office 365 ProPlus ソフトウェアをインストールまたは展開し、ライセンス認証されたユーザー数を確認します。
  - **Azure Active Directory** -セルフサービスのパスワードのリセット、サインインのカスタマイズ、アプリの追加などの一般的な Azure AD タスクにすばやくアクセスできます。
- [**ユーザーのエクスポート**] クイックタスクが [アクティブなユーザー] ページのコマンドバーに追加されました。 ![ユーザーがコマンドバーの [更新] の横にエクスポートされたことを示す画面キャプチャ。](media/M365PreviewExportUsersQT.png)
- 購入サービスには11月の追加の機能拡張があります。 これらはすべて良好です。 そのためには、課金 > 購入サービスに移動します。 <br>![3つの新しい拡張機能を備えた [購入サービス] ページを表示している画面取り込み。](media/M365PreviewBillingPurchasServices.png)
  > 1.  ページの上部で、ビジネスプランと教育プランの間でピボットを行うことができます。 これは、探しているものを見つけるのに役立ちます。
  > 2.  検索ボックスを追加して、探しているものを見つけるために無限にスクロールする必要がないようにしました。
  > 3.  最後に、展開したり折りたたんだりできるセクションに購入できるプランを論理的に並べ替えています。 **Office アプリとサービス**を購入する必要はありませんか? そのセクションを折りたたんで、Microsoft 365 プランに注目します。

## <a name="october-2018"></a>2018 年 10 月

- Microsoft 365 管理センターでサブスクリプションを購入するための新しい方法が用意されています。 [**購入サービス**] に移動すると、完全に再設計されたショッピング環境が表示され、検索、プランの比較、およびサブスクリプションの購入を容易にする新機能が向上します。 <br/>![Microsoft 365 管理センタープレビューでの購入サービスプランの比較](media/0ad5acf1-8ffc-44bf-aac2-c21269910e65.png)

- [**サービスと**アドイン] ページは新しいルックアンドフィールに更新されましたが、すべての機能はまったく同じように動作します。 たとえば、フィルターを適用してリストをコンパクトビューに変更するなど、新しい事柄を確認するには、コマンドバーを確認する必要があります。 ナビゲーションウィンドウで、[**設定** \> ] [**サービス&amp;アドイン**] に移動して確認します。 <br/>![Microsoft 365 管理センタープレビューの [設定とアドイン] ページ](media/af9a3ed3-bfe6-438b-b4b5-2ca4164f8e3f.png)

## <a name="september-2018"></a>2018 年 9 月

- **ホームの機能強化**ホームページは完全に再作成されているため、試してみることはできず、お客様のご意見をお待ちしています。 これは個人のホームページで、組織内の他の管理者のホームページには影響しません。 <br/>![画面キャプチャ: Microsoft 365 管理センタープレビューのホームページ。](media/d1a497f1-30e6-4c1a-9bb2-d4d7895906da.png)

  - ホームページカードの追加、削除、およびホームページへの移動を行うことができます。 右上隅にある [ **+ カードの追加**] を選択し、カードライブラリで利用可能なカードを確認してから、ホームページ上の任意の場所にドラッグします。 着陸した場所は好きではありませんか? 目的の場所にドラッグします。 これ以上保存する必要はありません。

  - カードをセクションごとにグループ化して、組織を次のレベルに移動します。 次の2つの既定のセクションが作成されています。**基本**と**デバイスと更新プログラム。** すぐに、セクションを追加、削除、および名前変更して、機能するホームページを設計することができます。

  - ほとんどのカードは、わかりやすく、実用的なものであり、日常のタスクを迅速かつ効率的に実行するのに役立つジャストインタイムの情報が提供されることを意味します。

  - ナビゲーションウィンドウは再編成されており、クイックタスク用にカスタマイズすることができます。 ナビゲーションウィンドウの下部にある [**編集**] を選んで、カスタマイズを開始します。 <br/>![Microsoft 365 管理センタープレビューナビゲーションウィンドウ](media/7d5568cc-9ce8-40b2-aa87-e777fcd455e3.png)

- **請求書**Microsoft 365 管理センターの [課金] セクションには、次のような大きな変更点がいくつかあります。 Microsoft または他のベンダーから購入したサブスクリプションとアプリを管理できます。これには、在庫管理、注文履歴、支払い方法、請求に関する新しいエクスペリエンスが含まれます。

  - サブスクリプション管理は再設計されており、アプリ、ソフトウェア、アドインなど、より多くの製品の種類に対して新しいインベントリ管理を提供しています。追加された新しい製品の種類のサポートをより深く反映するために、**製品&amp;サービス**と呼ばれます。 製品とサービスのカードまたは表を表示するかどうかを選択できるようになります。また、検索とフィルターを使用して、探しているものをすばやく見つけることができます。 <br/>![Microsoft 365 管理センタープレビューの製品とサービス](media/f1bba7dc-2034-4a53-b4ad-b7fbef3fb442.png)

  - ユーザーに製品ライセンスを割り当てることがこれまでになく簡単になりました。 購入した製品を選択してから、製品を割り当てることができます。 特定の製品が既に割り当てられているユーザーを確認することもできます。 <br/>![Microsoft 365 管理センタープレビューでの請求書からの製品詳細](media/82422685-e1b7-4f3f-aea8-1819df9d9311.png)

  - **スイッチプラン**は、* * アップグレード * * となり、最後にサブスクリプションのアップグレードが実行されるため、アップグレードの完了を監視する必要はありません。 最後に、アップグレードのライセンスの制限がなくなりました。

- **ユーザー管理**ユーザー管理に多くの変更が加えられ、作業効率を向上させることができました。 * * アクティブユーザー * * ページとユーザーの詳細ウィンドウを更新しました。 すばらしい変更のいくつかを次に示します。

  - [**アクティブなユーザー** ] ページには新しいデザインと新しいレイアウトがあります。 リストが表示されないようにする [**ビューの変更**] ( **[フィルター**] の横) を選択すると、リストを**標準リスト**から**圧縮リスト**に変更できます。 コンパクトリストは、組織内のユーザー数が多い場合に特に役立ちます。 <br/>![プレビューの [アクティブなユーザー] ページ](media/4dd9f1c9-ef23-4194-9ff8-5206ba2930f8.png)

  - コマンドバー (**検索**の場所) は動的であり、選択したユーザー数に応じて、ユーザー、1人、または複数のユーザーがいないと、アクションの一覧が変わります。 また、[**検索**] の横に選択したユーザーの数も表示されます。 <br/>![Microsoft 365 preview Active users コマンドバー。](media/9576bd19-31a5-4e59-a8dd-0663a833def0.png)

  - 1つまたは複数のユーザーのパスワードをリストビューから直接リセットするなどのクイックタスクを実行できます。

  - ユーザーの詳細と設定は、**アカウント**、**デバイス**、**ライセンス**、**アプリ**、**メール**、および**OneDrive**によって、次のタブで構成されるようになりました。 以前は、これらの設定の多くは複数の画面間に配置されていました。 <br/>![管理センタープレビューの [ユーザー詳細] ウィンドウ](media/60c6b9d3-d62f-4b8d-97db-41ff247b1bf0.png)<br/>
