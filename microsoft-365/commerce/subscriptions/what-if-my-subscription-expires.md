 ...
 **想定される動作:** 期限切れステージのときにサブスクリプションを再アクティブ化しなければ、無効ステージに移行します。期間は、ほとんどのサブスクリプションおよびほとんどの国と地域で 90 日間です。 ボリューム ライセンス製品の場合、期限切れステージは 30 日間です。
...
- **試用版を終了するか、期限切れにします。** Microsoft 365 を購入しない場合は、試用期間を終了するか、[キャンセルすることができます](cancel-your-subscription.md)。 保持するデータをバックアップします。 30 日間の期限切れステージが過ぎるとすぐに、試用版のアカウントの情報とデータが完全に消去されます。
