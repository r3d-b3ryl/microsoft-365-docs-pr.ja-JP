...
> [!NOTE]
> Advanced eDiscovery の大きなケース形式のパブリック プレビュー中に [、For](advanced-ediscovery-large-cases.md) **Review** フィルター クエリは、2021 年 11 月 4 日より前に作成されたレビュー セットの Teams 会話を返しません。 この問題は解決されました。 つまり、For **Review** クエリをレビュー セットに再適用すると、すべてのユーザーの会話が含まれるため、フィルター クエリに一致するアイテムTeams表示される可能性があります。
