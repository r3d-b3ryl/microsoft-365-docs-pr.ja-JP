...
- シミュレーション モード:
  ...
  - ソース ビューでファイルやメールのコンテンツを表示する場合は、**コンテンツ エクスプローラーのコンテンツ閲覧者** の役割が必要です。既定では、全体管理者にはこの役割はありません。このアクセス許可が与えられていない場合は、[**一致したアイテム**] タブからアイテムを選んだときに、[プレビューアー] ウィンドウが表示されません。
