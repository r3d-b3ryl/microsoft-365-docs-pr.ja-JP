---
title: SharePoint ドキュメントのライフサイクルを管理に保持ラベルを使用する
f1.keywords:
- NOCSH
ms.author: cabailey
author: cabailey
...
Microsoft Purview コンプライアンス ポータルを使用して、次の[保持ラベル](retention.md#retention-labels)を作成します。
...
次のスクリーンショットは、Microsoft Purview コンプライアンス ポータルで製品の仕様の保持ラベルを作成するときの設定を示しています。 保持ラベルを作成するときに、*製品の中止* のイベントの種類を作成できます。 次のセクションの手順を参照してください。
...
3. **[イベントの種類に名前を付ける]** ページで、**[製品の停止 ]** とオプションの説明を入力します。その後、**[次へ]**、**[送信]**、**[完了]** の順に選択します。
...
1. <a href="https://go.microsoft.com/fwlink/p/?linkid=2077149" target="_blank">Microsoft Purview コンプライアンス ポータル</a>で、**[レコード管理]** > **[ラベル ポリシー]** > **[ラベルの自動適用]** の順に移動します。
...
7 日後に、Microsoft Purview コンプライアンス ポータルの [[アクティビティ エクスプローラー]](data-classification-activity-explorer.md) を使用して、作成した自動適用ラベル ポリシーによって保持ラベルが製品ドキュメントに自動的に適用されたことを確認します。
...
Microsoft Purview コンプライアンス ポータルでは、**[レコード管理]** > **[イベント]** に移動することで、手動でイベントを作成できます。 イベントの種類を選択し、正しい資産 ID を設定して、イベントの日付を入力します。 詳細については、[「イベントが発生したときに保持を開始 」](event-driven-retention.md) を参照してください。
...
- **名前**: このパラメーターでは、Microsoft Purview コンプライアンス ポータルで作成するイベントの名前を指定します。 このシナリオでは、名前は「Cessation Production *xxx*」 です。*xxx* は、前に作成した **ProductName** 管理プロパティの値です。
...
この時点で、保持ラベルの作成と自動適用、およびフローの構成と作成が済んでいます。[製品] リストで回転ウィジェット製品に対応する **[生産中]** 列の値が、**_[はい]_*_ から _*_[いいえ]_*_ に変更されると、イベントを作成するフローがトリガーされます。このイベントを Microsoft Purview コンプライアンス ポータルで確認するには、_*[レコード管理]** > **[イベント]** の順に移動します。

[![Microsoft Purview コンプライアンスポータルの [イベント] ページに表示されている、フローによってトリガーされたイベント。](../media/SPRetention28.png)](../media/SPRetention28.png#lightbox)
