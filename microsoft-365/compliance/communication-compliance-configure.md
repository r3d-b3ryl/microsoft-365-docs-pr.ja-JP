...
- **手順 3 (必須)**:[通信コンプライアンスポリシーをセットアップする](#step-3-create-a-communication-compliance-policy-required)
    ...
    コミュニケーションコンプライアンスポリシーは、Microsoft 365 コンプライアンスセンターで作成します。 これらのポリシーでは、組織内で検討する対象となる通信を定義し、レビューを実行するユーザーを指定します。 通信には、電子メール、Microsoft Teams、Skype for Business、サードパーティのプラットフォーム通信 (Facebook、Twitter など) などが含まれます。
...
- **手順 6 (オプション)**:[通信コンプライアンスポリシーの監査を有効にする](#step-6-enable-auditing-for-your-communication-compliance-policies-optional)

    組織の監査を有効にして、通信コンプライアンスポリシーの管理アクティビティを記録します。
...
## <a name="step-6-enable-auditing-for-your-communication-compliance-policies-optional"></a>手順 6: 通信コンプライアンスポリシーの監査を有効にする (オプション)

ポリシーのテストが完了したら、通信コンプライアンス管理に関連付けられたアクティビティが記録されるように監査を有効にすることができます。 これは、定義済みの組織ポリシーに関連付けられているすべてのアクティビティ、または通信コンプライアンスポリシーが変更されるたびに、その概要になる場合があります。
...
監査をオンにするには、Office 365 セキュリティ & コンプライアンスセンターの [**監査ログの検索**] ページで、[**ユーザーと管理者のアクティビティの記録を開始**する] をクリックします。 このリンクが表示されない場合は、組織の監査が既に有効になっています。 監査をオンにすると、監査ログが準備されていて、準備が完了してから数時間で検索を実行できることを示すメッセージが表示されます。 これを行う必要があるのは 1 回だけです。 監査ログの詳細については、「 [Search the audit log](search-the-audit-log-in-security-and-compliance.md)」を参照してください。
