...
[組み込みのトレーニング可能な分類子とグローバル分類子は、](/microsoft-365/compliance/classifier-learn-about) 組織内のすべての通信チャネルで送受信されたメッセージをスキャンして、さまざまな種類のコンプライアンスの問題を検出します。 分類子は、人工知能とキーワードの組み合わせを使用して、ハラスメント対策ポリシーに違反する可能性のある言葉を識別します。 組み込みの分類子は、現在、複数の言語でメッセージ キーワードの識別をサポートしています。
