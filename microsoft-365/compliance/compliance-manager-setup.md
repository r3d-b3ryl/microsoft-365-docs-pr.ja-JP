...
組織でグローバル管理者の役割を持つユーザーが、コンプライアンス マネージャーのユーザー権限を設定できます。 アクセス許可は、Microsoft 365 コンプライアンス センター (Microsoft 365 コンプライアンス センター) のAzure Active Directory設定Azure AD。
...
アクセス許可を設定し、役割を割り当てるには、Microsoft 365 コンプライアンス センター手順に従います。
...
コンプライアンス マネージャーは、情報ガバナンス、情報保護、データ損失防止、通信コンプライアンス、インサイダー リスク管理など、組織がサブスクライブしている他の Microsoft 365 コンプライアンス ソリューションからのシグナルを検出します。 各改善アクションの詳細ページで、[テスト]  タブの [テスト]  ロジック フィールドに、アクションがコンプライアンス スコアに向かってポイントを渡して獲得するために、他のソリューションで必要な内容が表示されます。

コンプライアンス マネージャーは、Microsoft Secure Score によって監視される補完的な改善アクションからのシグナル [も検出します](../security/defender/microsoft-secure-score.md)。 コンプライアンス マネージャーは、これらのシグナルを使用して、特定の改善アクションを自動的にテストし、コンプライアンス活動の効率を最大化できます。 改善アクションが正常にテストおよび実装されると、完全なポイントが受け取り、全体的なコンプライアンス スコアが与されます。

コンプライアンス マネージャーを初めて利用する組織では、自動テストが既定でオンになっています。 データを最初に展開Microsoft 365、Office 365データを完全に収集し、コンプライアンス スコアに組み込むには約 7 日かかっています。 自動化されたテストがオンになっていると、アクションのテスト日は更新されませんが、テストの状態は更新されます。 新しい評価を作成した場合、スコアには Microsoft コントロール スコアとセキュア スコアの統合が自動的に含まれます。

#### <a name="manage-automated-testing-settings"></a>自動テスト設定の管理
...
**詳細情報**
- [継続的な監視がコンプライアンス スコアに貢献する方法について詳しくは、ご覧ください](compliance-score-calculation.md#how-compliance-manager-continuously-assesses-controls)。
- [改善アクションのテスト ソースを指定する方法について詳しくは、をご覧ください](compliance-manager-improvement-actions.md#update-testing-source)。
...
コンプライアンス マネージャーのダッシュボードには、あなたの主要な改善のための処置が表示されます。 すべての改善アクションを表示するには、ダッシュボードの [改善アクション] タブを選択して、改善アクション ページに移動します。 ダッシュボードの主要な改善 **アクション** の一覧の下にある [すべての改善アクションを表示する] を選択して、改善アクション ページに移動することもできます。
...
改善のための処置ページに長いアクションのリストがある場合は、表示をフィルター処理するとよいでしょう。 アクション リストの右上にある **[フィルター]** を選択します。 [フィルター **] フライ** アウト ウィンドウが表示されたら、使用可能なオプションから条件を選択します。 また、右上の **[グループ]** を選択することで、表示をカスタマイズすることができます。 ドロップダウン メニューから、グループ別、解決方法別、カテゴリ別、アクションの種類別、状態別の表示を選択します。
...
- **アクションの** 種類: 改善アクションが技術的なアクションであるかどうかを示します。つまり、技術的なソリューションの外部で実装されるソリューションまたは製品、または非技術的なアクション内で実装できるかどうかを示します。
- **割り当** て: 該当する場合、このアクションが割り当てられているユーザー
...
2. [フィルター **] フライ** アウト ウィンドウで、目的の条件 (規制、ソリューション、アクションの種類、グループ、カテゴリ) の横にチェックを行います。
