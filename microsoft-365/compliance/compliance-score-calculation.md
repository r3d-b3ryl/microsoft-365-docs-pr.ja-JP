...
コンプライアンス マネージャーは、Microsoft 365 環境全体を自動的にスキャンしてシステム設定を検出し、技術的なアクションの状態を継続的かつ自動的に更新します。 Microsoft セキュア スコアは、監視を実行する基になるエンジンです。

アクションの状態は、ダッシュボードで 24 時間ごとに更新されます。 推奨事項に従ってコントロールを実装すると、通常、コントロールの状態は翌日更新されます。
