...
2. 左側のナビゲーション ウィンドウで、[すべて **表示]** をクリックし、[電子情報開示] > **クリックします**。
...
7. [場所 **の選択] ウィザード** ページで、保留にするコンテンツの場所を選択します。 メールボックス、サイト、およびパブリック フォルダーを保留にすることができます。
   ...
   2. **SharePoint サイト**: トグルを **On** に設定し、[**サイトの選択]** をクリックして、ホールドする SharePoint サイトと OneDrive アカウントを指定します。 ホールドを適用する各サイトの URL を入力します。 また、Microsoft チーム、SharePointグループ、またはグループのOffice 365サイトの URL をYammerすることもできます。
...
- Team または Office 365 グループのメンバーの一覧を取得するには、グループの [グループ]<a href="https://go.microsoft.com/fwlink/p/?linkid=2052855" target="_blank"></a>ページでプロパティをMicrosoft 365 管理センター。 または、Exchange Online PowerShell で次のコマンドを実行できます。
