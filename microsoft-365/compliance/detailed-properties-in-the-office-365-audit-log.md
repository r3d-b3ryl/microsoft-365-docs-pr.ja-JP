...
|ID|監査レコード エントリの ID。 ID は、監査ログのレコードを一意に識別します。<sup>1</sup>|すべて|
...
|UserType|操作を実行したユーザーの種類。次の値によって、ユーザーの種類が示されます。<br/> <br/> **0** - 標準のユーザー。 <br/>**2** - Office 365 組織の管理者。<sup>2</sup> <br/>**3** - Microsoft データセンター管理者またはデータセンターのシステム アカウント。 <br/>**4** - システム アカウント。 <br/>**5** - アプリケーション。 <br/>**6** - サービス プリンシパル。<br/>**7** - カスタム ポリシー。<br/>**8** - システム ポリシー。|すべて|
|Version|ログに記録された (**Operation** プロパティで識別された) アクティビティのバージョン番号を示します。|すべて|
|ワークロード|アクティビティが発生した Office 365 サービス。|すべて|
||||

> [!NOTE]
><sup>1</sup> 重複する監査レコード (**ID** プロパティの値が同じ) が監査ログに表示される場合があります。 これは、監査イベントを見逃さないためのエラー修正方法として設計されています。<br/><br/><sup>2</sup> Azure Active Directory 関連のイベントの場合、管理者の値は監査レコードで使用されません。 管理者によって実行されたアクティビティの監査レコードは、通常のユーザー (たとえば、**UserType: 0**) がアクティビティを実行したことを意味します。 **UserID** プロパティは、アクティビティを実行したユーザー (通常のユーザーまたは管理者) を識別します。
