...
- メタデータフィールドの名前 ([**フィールド名]** 列)。

- [検索可能な**フィールド名]** 列で、[校閲セットクエリ](review-set-search.md)を実行するときに検索できるプロパティの名前を指定します。 空白セルは、レビューセットクエリでフィールドを検索できないことを意味します。

- [エクスポートされた**フィールド名]** 列に、ドキュメントをエクスポートするときに含まれるメタデータフィールドの名前を指定します。  空白セルは、メタデータフィールドがエクスポートされたメタデータに含まれていないことを意味します。

- 校閲セット内の選択したドキュメントのファイルメタデータを表示するときに表示されるメタデータフィールドの名前 ([**表示フィールド名]** 列)。 空白セルは、ドキュメントのファイルメタデータを表示するときに、メタデータフィールドが含まれていないことを示します。
...
| File クラス | FileClass | File_class | File クラス | SharePoint および OneDrive のコンテンツの場合:**ドキュメント**;Exchange からのコンテンツの場合: * * 電子メールまたは**添付ファイル**。 |
...
| 場所 | 場所 | 場所 | 場所 | ドキュメントのソースとなる場所の種類を示す文字列。<br />O365 以外の > インポートされたデータ<br />Teams-> Teams<br />EXO-> Exchange<br />SPO-> SharePoint<br />OneDrive for business-OneDrive > |
...
| Sender | Sender | Email_sender | Sender | メッセージの種類の Sender (差出人) フィールド。  Format は**DisplayName \<[smtpaddress>**] です。 |
...
| タイトル | タイトル | Doc_title | 役職 | ドキュメントのメタデータからのタイトル。 |
| 宛先 | 宛先 | Email_to | 宛先 | メッセージの種類の "宛先" フィールド  形式は、[ **\<smtpaddress>** |
