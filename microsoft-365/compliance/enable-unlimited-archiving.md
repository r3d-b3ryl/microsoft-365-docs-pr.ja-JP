...
---
# <a name="enable-unlimited-archiving---admin-help"></a>無制限アーカイブを有効にする - 管理者ヘルプ
...
特定のユーザーと保持中のユーザーのメールボックス、またはアイテム保持ポリシーに割り当てられたユーザーの自動拡張アーカイブを有効にした場合、次の 2 つの構成が変更されます。
...
- ユーザーがアーカイブ メールボックス内の追加の記憶域にあるアイテムにアクセスするために使用できる Outlook クライアントの一覧については、「無制限アーカイブの概要」の「自動拡張アーカイブ内のアイテムにアクセスする Outlook の要件」セクションを参照[してください。](unlimited-archiving.md#outlook-requirements-for-accessing-items-in-an-auto-expanded-archive)
