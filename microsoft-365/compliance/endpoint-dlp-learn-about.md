...
[Microsoft Defender for Endpoint](../security/defender-endpoint/configure-machines-onboarding.md) を介してデバイスをオンボードした場合、それらのデバイスは自動的にデバイスの一覧に表示されます。 これは、Defender へのオンボードでもデバイスを DLP にオンボードするためです。 エンドポイント DLP を使用するには、**デバイスの監視を有効にする** だけです。 .
