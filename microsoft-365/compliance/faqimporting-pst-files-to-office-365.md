 ...
 **PST インポートプロセスが重複した電子メールアイテムを処理する方法**

PST インポートプロセスは、重複したアイテムをチェックし、対象のメールボックスまたはターゲットアーカイブのターゲットフォルダーに一致するアイテムが存在する場合は、そのアイテムを PST ファイルからメールボックスまたはアーカイブにコピーしません。 以前のインポートジョブで指定したものとは別の PST ファイルを再インポートして、(PST インポートマッピングファイルの TargetRootFolder プロパティを使用して) 別のターゲットフォルダーを指定した場合、PST ファイル内のすべてのアイテムが reimported されます。
