...
インサイダー リスク管理の構成を開始するには、一覧から推奨事項を選択します。 推奨される各アクションでは、要件、期待する内容、組織で機能を構成した場合の影響など、推奨事項に必要なアクティビティについて説明します。   推奨される各アクションは、構成時に自動的に完了としてマークされます。または、構成時にアクションを完了として手動で選択する必要があります。
...
## <a name="step-2-enable-the-microsoft-365-audit-log-optional"></a>手順 2: 監査ログMicrosoft 365有効にする (オプション)
