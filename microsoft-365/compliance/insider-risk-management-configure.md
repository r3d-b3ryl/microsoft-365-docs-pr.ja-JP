...
推奨されるアクションは、組織がインサイダーリスク管理を迅速に行うのに役立ちます。 [概要] **ページに含** まれる推奨アクションは、ポリシーを構成および展開する手順を説明するのに役立ちます。
...
| 分析インサイトにアクセスする | はい | はい | はい | 不要 | 不要 |
| アラートの&調査にアクセスする | はい | 不要 | はい | はい | 不要 |
| ケース&調査にアクセスする | はい | 不要 | はい | はい | 不要 |
| Access & コンテンツ エクスプローラーを表示する | はい | 不要 | 不要 | はい | 不要 |
| 通知テンプレートの構成 | はい | 不要 | はい | はい | 不要 |
| 監査ログ&を表示する | はい | 不要 | 不要 | 不要 | はい |
