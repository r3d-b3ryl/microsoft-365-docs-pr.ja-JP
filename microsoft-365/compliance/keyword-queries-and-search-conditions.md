...
|ファイルの種類|ファイルの拡張子。例: docx、one、pptx、xlsx など。 これは、FileExtension サイト プロパティと同じプロパティです。 <br/><br/> **注:** 検索クエリに **Equals** または **Equals** 任意の演算子を使用して File 型条件を含める場合、ファイルの種類のすべてのバージョンを返すプレフィックス検索 (ファイルの種類の末尾にワイルドカード文字 ( * ) を含める) を使用することはできません。 この場合、ワイルドカードは無視されます。 たとえば、条件を含める場合 `Equals any of doc*` は、拡張子が付くファイル `.doc` だけが返されます。 拡張子が付くファイル `.docx` は返されません。 ファイルの種類のすべてのバージョンを取得するには、キーワード *クエリで property:value ペア* を使用します。たとえば、 `filetype:doc*` .|
|||
