...
個々のドキュメントにユーザーが保持ラベルを適用できるようにするだけでなく、既定の保持ラベルを SharePoint ライブラリ、フォルダー、またはドキュメント セットに適用して、その場所にあるすべてのドキュメントに既定の保持ラベルを設定することもできます (既定のラベルは E5 の機能)。
...
- ライブラリ、フォルダーまたはドキュメント セット内のすべてのアイテムは、同じ保持ラベルを自動的に取得します。ただし、明示的に保持ラベルが適用されたアイテムは**除きます**。 明示的にラベルが付いたアイテムは、既存のラベルを保持します。 詳細については、下記の[保持の原則または優先順位](#the-principles-of-retention-or-what-takes-precedence)を参照してください。
...
- ライブラリ、フォルダー、またはドキュメント セットの既定の保持ラベルがコンテンツをレコード (*またはレコード ラベル*) と宣言している場合は、次の特性に適用されます。

   - 既定の保持ラベルからコンテンツをレコードとして宣言していないラベルに変更した場合、アイテムが既存の既定のレコード ラベルを保持します。 新しい既定の保持ラベルはこれらのアイテムには適用されません。 サイト コレクション管理者は、保持ラベルを明示的に削除または変更する必要があります。

   - コンテンツをレコードとして宣言する既定の保持ラベルを削除しても、ライブラリ、フォルダー、またはドキュメント セット内のアイテムのレコード ラベルは削除されません。 サイト コレクション管理者は、保持ラベルを明示的に削除する必要があります。
