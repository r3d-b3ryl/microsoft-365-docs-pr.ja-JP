...
- **コンテンツをレコードとして分類する**。 コンテンツを[レコード](records.md)としてマークする保持ラベルを作成および発行します。エンドユーザーが適用したり、機密情報、キーワード、またはコンテンツ タイプを識別して[自動適用](labels.md#applying-a-retention-label-automatically-based-on-conditions)したりすることができます。
...
- **レコード ラベル内で保持と削除のポリシーを確立します**。 最終更新日や作成日などのさまざまな要因に基づき、[保持](create-retention-policies.md#retaining-content-for-a-specific-period-of-time)および[廃棄](create-retention-policies.md#deleting-content-thats-older-than-a-specific-age)の期間を定義します。
...
Microsoft 365 でのレコード管理を始める準備ができている場合は、「[レコードについての詳細](records.md)」を参照してください。
