...
タイマー ジョブは、アイテム保管ライブラリを定期的にクリーンアップします。 このジョブはサイトの保持ポリシーで使用されたすべてのクエリに対してアイテム保管ライブラリのすべてのコンテンツが比較されます。 構成された保持期間よりも古いコンテンツは、アイテム保管ライブラリから削除されます。元の場所がまだそこにある場合は削除されます。 このタイマー ジョブは 7 日ごとに実行されます。つまり、コンテンツが削除されるまでに最大 7 日かかることがあります。
...
- [**特定の場所を選択する**] > [**SharePoint サイト**] または [**OneDrive アカウント**]
