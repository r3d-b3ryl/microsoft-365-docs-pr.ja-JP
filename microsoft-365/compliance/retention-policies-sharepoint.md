...
- 保持ラベルが自動的に適用されるクラウドの添付ファイルとして共有されたファイル
...
ファイルをアイテム保管ライブラリにコピーする場合のこの動作は、アイテム保持ポリシーが適用されたときに存在するコンテンツに適用されます。加えて、保持ポリシーの場合、ポリシーに含まれた後にサイトに作成または追加された新しいコンテンツは、アイテム保管ライブラリに保持されます。 ただし、新しいコンテンツは、最初に編集されたときにアイテム保管ライブラリにコピーされるのではなく、削除されたときにのみコピーされます。 ファイルのすべてのバージョンを保持するには、[バージョン管理](#how-retention-works-with-document-versions)を有効にする必要があります。
