...
静的ポリシースコープを使用し、保持設定を "**すべての受信者**" に適用すると、すべての [アクティブなメールボックス](create-and-manage-inactive-mailboxes.md)が含まれます。 ただし、この規定値を変更して[特定の追加または除外](#a-policy-with-specific-inclusions-or-exclusions)を構成すると、非アクティブなメールボックスはサポートされず、それらのメールボックスに保持設定が適用または除外されません。

静的ポリシー スコープで含める受信者または除外する受信者を選択した場合は、配信グループとメールが有効なセキュリティ グループを 1 つずつ選択するのではなく、複数の受信者を効率的に選択する方法として選択できます。 このオプションを使用すると、バックグラウンドでは、これらのグループは構成時に自動的に展開され、グループ内のユーザーのメールボックスを選択します。 これらのグループのメンバーシップが後で変更されても、既存のアイテム保持ポリシーは自動的に更新されません。
...
コンテンツの保持ポリシーを構成するときは、アイテムを特定の日数、月数、または年数の間保持することを選択します。 または、アイテムを永久に保持します。 保存期間は、アイテム保持ポリシーが適用された時点からではなくコンテンツの経過時間から計算されます。
...
どちらの場合も、ポリシーによってアイテムを削除する場合、アイテム保持ポリシーに指定した期間は、ポリシーが割り当てられた時点からではなく、アイテムが作成または変更された時点から計算されることを理解しておくことが重要です。
