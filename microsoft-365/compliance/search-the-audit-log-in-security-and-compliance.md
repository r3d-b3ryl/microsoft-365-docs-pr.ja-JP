...
- 監査対象アクティビティがユーザーまたは管理者によって実行されると、監査レコードが生成され、組織の Office 365 監査ログに格納されます。監査レコードの保持期間 (および監査ログで検索可能な期間) は、Office 365 または Microsoft 365 サブスクリプション、具体的には、特定のユーザーに割り当てられたライセンスの種類によって異なります。

  - **Office 365 と Microsoft 365 E3:** 監査レコードは 90 日間保持されます。 つまり、過去 90 日間に実行されたアクティビティの監査ログを検索できるのです。
  ...
  - **Office 365 または Microsoft 365 E5 またはMicrosoft 365 E5 コンプライアンス アドオン ライセンスを持っているユーザー:** Azure Active Directory、Exchange、および SharePoint アクティビティの監査レコードは、デフォルトで1年間保持されます。 また、組織では、監査ログの保持ポリシーを作成して、その他のサービスのアクティビティの監査レコードを最大 1 年間保持します。 詳細については、「[監査ログ保持ポリシーを管理する](audit-log-retention-policies.md)」を参照してください。
...
前述のとおり、監査レコードの保持期間は、組織の Office 365 または Microsoft サブスクリプションによって異なります。
