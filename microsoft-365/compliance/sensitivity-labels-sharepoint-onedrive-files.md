...
このプレビューでは、これらのすべての機能を有効にします。 機密ラベルをユーザーに表示することに加えて、クラウドベースのキーを使用した暗号化を含む、新しいファイルと変更されたファイルについては、次のようにします。
...
> [!NOTE]
> クラウドベースのキーで暗号化が適用されておらず、オンプレミスキーであっても、"自分のキーを保持する" と呼ばれるキー管理トポロジ (HYOK) では、ファイルコンテンツを処理するための SharePoint の動作はこのプレビューでは変更されません。
>
...
このプレビューを有効にすると、web 上の Office を使用して適用される機密ラベルを監視するための3つの新しい[監査イベント](search-the-audit-log-in-security-and-compliance.md#sensitivity-label-activities)が利用可能になります。
- **ファイルに適用された機密ラベル**
- **ファイルに適用された機密ラベルの変更**
- **ファイルから削除された機密ラベル**
...
1. グローバル管理者または SharePoint 管理者権限を持つ職場または学校のアカウントを使用して、SharePoint に接続します。 方法の詳細については、「[SharePoint Online 管理シェルの使用を開始する](https://docs.microsoft.com/powershell/sharepoint/sharepoint-online/connect-sharepoint-online)」を参照してください。
