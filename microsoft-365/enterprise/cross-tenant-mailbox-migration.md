...
## <a name="prerequisites"></a><span data-ttu-id="c5d49-126">前提条件</span><span class="sxs-lookup"><span data-stu-id="c5d49-126">Prerequisites</span></span>
...
<span data-ttu-id="c5d49-155">ソース テナントを準備します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-155">Prepare the source tenant:</span></span>
...
5. <span data-ttu-id="c5d49-165">次のパラメーターと値を使用してスクリプトを実行します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-165">Run the script with the following parameters and values.</span></span>
    ...
    | <span data-ttu-id="c5d49-179">-ResourceTenantId</span><span class="sxs-lookup"><span data-stu-id="c5d49-179">-ResourceTenantId</span></span>                           | <span data-ttu-id="c5d49-180">ソース テナント組織 ID (GUID)。</span><span class="sxs-lookup"><span data-stu-id="c5d49-180">Source tenant organization ID (GUID).</span></span> | <span data-ttu-id="c5d49-181">必須</span><span class="sxs-lookup"><span data-stu-id="c5d49-181">Required</span></span> |
    ...
    | <span data-ttu-id="c5d49-209">-KeyVaultAuditStorageResourceGroup</span><span class="sxs-lookup"><span data-stu-id="c5d49-209">-KeyVaultAuditStorageResourceGroup</span></span>          | <span data-ttu-id="c5d49-210">Key Vault 監査ログを格納するためのストレージ アカウントを含むリソース グループ。</span><span class="sxs-lookup"><span data-stu-id="c5d49-210">The resource group that contains the storage account for storing Key Vault audit logs.</span></span> | <span data-ttu-id="c5d49-211">省略可能</span><span class="sxs-lookup"><span data-stu-id="c5d49-211">Optional</span></span> |
    ||||
...
7. <span data-ttu-id="c5d49-216">URL はリモート PowerShell セッションに表示されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-216">A URL will be displayed in the Remote PowerShell session.</span></span> <span data-ttu-id="c5d49-217">テナントの同意のために提供されたリンクをコピーし、Web ブラウザーに貼り付けます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-217">Copy the link provided for your tenant consent and paste it into a Web browser.</span></span>
...
## <a name="prepare-target-user-objects-for-migration"></a><span data-ttu-id="c5d49-273">移行のターゲット ユーザー オブジェクトを準備する</span><span class="sxs-lookup"><span data-stu-id="c5d49-273">Prepare target user objects for migration</span></span>
...
1. <span data-ttu-id="c5d49-279">移動元の組織から移動するメールボックスの場合は、移動先の組織で MailUser オブジェクトを準備する必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-279">For any mailbox moving from a source organization, you must provision a MailUser object in the Target organization:</span></span> 
   ...
   - <span data-ttu-id="c5d49-280">移動先 MailUser には、移動元のメールボックスから次の属性を設定するか、新しいユーザー オブジェクトで割り当てる必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-280">The Target MailUser must have these attributes from the source mailbox or assigned with the new User object:</span></span>
     ...
     | <span data-ttu-id="c5d49-315">EmailAddresses</span><span class="sxs-lookup"><span data-stu-id="c5d49-315">EmailAddresses</span></span>        | <span data-ttu-id="c5d49-316">x500:/o=First Organization/ou=Exchange Administrative Group (FYDIBOHF23SPDLT)/cn=Recipients/cn=d11ec1a2cacd4f81858c8190</span><span class="sxs-lookup"><span data-stu-id="c5d49-316">x500:/o=First Organization/ou=Exchange Administrative Group (FYDIBOHF23SPDLT)/cn=Recipients/cn=d11ec1a2cacd4f81858c8190</span></span>  |
...
<span data-ttu-id="c5d49-390">パラメーターを使用 `-flags` します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-390">Use the `-flags` parameter.</span></span> <span data-ttu-id="c5d49-391">次に例を示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-391">Here is an example.</span></span>
...
<span data-ttu-id="c5d49-398">ドメインを所有できるのは 1 つのテナントだけのため、メールボックスの移動が完了すると、以前のプライマリ SMTPAddress はターゲット テナントのユーザーに関連付けされません。新しいテナントに関連付けられているドメインのみ。</span><span class="sxs-lookup"><span data-stu-id="c5d49-398">Since only one tenant can own a domain, the former primary SMTPAddress will not be associated to the user in the target tenant when the mailbox move completes; only those domains associated with the new tenant.</span></span> <span data-ttu-id="c5d49-399">Outlook はユーザーの新しい UPN を使用してサービスに対する認証を行い、Outlook プロファイルはターゲット システム内のメールボックスと一致する従来のプライマリ SMTPAddress を検索する必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-399">Outlook uses the users new UPN to authenticate to the service and the Outlook profile expects to find the legacy primary SMTPAddress to match the mailbox in the target system.</span></span> <span data-ttu-id="c5d49-400">レガシ アドレスがターゲット システムに含ではなされていないので、Outlook プロファイルは新しく移動したメールボックスを検索するために接続されません。</span><span class="sxs-lookup"><span data-stu-id="c5d49-400">Since the legacy address is not in the target System the outlook profile will not connect to find the newly moved mailbox.</span></span> 

<span data-ttu-id="c5d49-401">この初期展開では、ユーザーは新しい UPN、プライマリ SMTP アドレスを使用してプロファイルを再構築し、OST コンテンツを再同期する必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-401">For this initial deployment, users will need to rebuild their profile with their new UPN, primary SMTP address and re-sync OST content.</span></span> 

> [!Note]
> <span data-ttu-id="c5d49-402">ユーザーのバッチ処理を完了に合わせて計画します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-402">Plan accordingly as you batch your users for completion.</span></span> <span data-ttu-id="c5d49-403">Outlook クライアント プロファイルを作成し、それ以降の OST ファイルと OAB ファイルをクライアントにダウンロードする場合は、ネットワークの使用率と容量を考慮する必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-403">You need to account for network utilization and capacity when Outlook client profiles are created and subsequent OST and OAB files are downloaded to clients.</span></span> 
 
<span data-ttu-id="c5d49-404">**テナント間の移行を設定または完了するには、どのような Exchange RBAC の役割のメンバーである必要がありますか。**</span><span class="sxs-lookup"><span data-stu-id="c5d49-404">**What Exchange RBAC roles do I need to be member of to set up or complete a cross-tenant move?**</span></span>
 
<span data-ttu-id="c5d49-405">メールボックス移動を実行する際の委任された職務の前提に基づく役割のマトリックスがあります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-405">There a matrix of roles based on assumption of delegated duties when executing a mailbox move.</span></span> <span data-ttu-id="c5d49-406">現在、2 つの役割が必要です。</span><span class="sxs-lookup"><span data-stu-id="c5d49-406">Currently, two roles are required:</span></span>  

- <span data-ttu-id="c5d49-407">1 つ目の役割は、テナント/組織の境界内または組織の境界にコンテンツを移動する承認を確立する 1 回設定タスクです。</span><span class="sxs-lookup"><span data-stu-id="c5d49-407">The first role is for a one-time setup task that establishes the authorization of moving content into or out of your tenant/organizational boundary.</span></span> <span data-ttu-id="c5d49-408">組織の管理からデータを移動する操作は、すべての企業にとって重要な問題です。組織管理者 (OrgAdmin) の最高の役割を割り当てました。</span><span class="sxs-lookup"><span data-stu-id="c5d49-408">As moving data out of your organizational control is a critical concern for all companies, we opted with the highest assigned role of Organization Administrator (OrgAdmin).</span></span> <span data-ttu-id="c5d49-409">この役割は、リモート組織での -MailboxMoveCapability を定義する新しい OrganizationRelationship を変更またはセットアップする必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-409">This role must alter or setup a new OrganizationRelationship that defines the -MailboxMoveCapability with the remote organization.</span></span> <span data-ttu-id="c5d49-410">OrganizationRelationhip の他の属性はフェデレーション共有管理者が管理できるのに対し、OrgAdmin だけが MailboxMoveCapability 設定を変更できます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-410">Only the OrgAdmin can alter the MailboxMoveCapability setting, while other attributes on the OrganizationRelationhip can be managed by the Federated Sharing administrator.</span></span> 
 
- <span data-ttu-id="c5d49-411">実際の移動コマンドを実行する役割は、下位レベルの関数に委任できます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-411">The role of executing the actual move commands can be delegated to a lower-level function.</span></span> <span data-ttu-id="c5d49-412">メールボックスの移動の役割には、パラメーターを使用してメールボックスを組織内外に移動する機能が割り当 `-RemoteTenant` てられます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-412">The role of Move Mailboxes is assigned the capability of moving mailboxes in or out of the organization by using the `-RemoteTenant` parameter.</span></span>  

<span data-ttu-id="c5d49-413">**変換されたメールボックスの targetAddress (TargetDeliveryDomain) に対して選択されている SMTP アドレスをターゲットにする方法 (MailUser 変換へ)**</span><span class="sxs-lookup"><span data-stu-id="c5d49-413">**How do we target which SMTP address is selected for targetAddress (TargetDeliveryDomain) on the converted mailbox (to MailUser conversion)?**</span></span>
 
<span data-ttu-id="c5d49-414">移動先オブジェクトの電子メール アドレス (proxyAddress) と一致して MailUser に変換する場合、MRS を使用して移動する Exchange メールボックスは、元のソース メールボックスで targetAddress を作成します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-414">Exchange mailbox moves using MRS craft the targetAddress on the original source mailbox when converting to a MailUser by matching an email address (proxyAddress) on the target object.</span></span> <span data-ttu-id="c5d49-415">このプロセスは、move コマンドに渡された -TargetDeliveryDomain 値を受け取り、ターゲット側のそのドメインに対応するプロキシを確認します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-415">The process takes the -TargetDeliveryDomain value passed into the move command, then checks for a matching proxy for that domain on the target side.</span></span> <span data-ttu-id="c5d49-416">一致が見つけると、一致する proxyAddress が使用され、変換されたメールボックス (現在は MailUser) オブジェクトに ExternalEmailAddress (targetAddress) が設定されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-416">When we find a match, the matching proxyAddress is used to set the ExternalEmailAddress (targetAddress) on the converted mailbox (now MailUser) object.</span></span>
 
<span data-ttu-id="c5d49-417">**メールボックスのアクセス許可の移行方法**</span><span class="sxs-lookup"><span data-stu-id="c5d49-417">**How do mailbox permissions transition?**</span></span>

<span data-ttu-id="c5d49-418">メールボックスのアクセス許可には、代理送信とメールボックス アクセスが含まれます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-418">Mailbox permissions include Send on Behalf of and Mailbox Access:</span></span> 

- <span data-ttu-id="c5d49-419">代理人として送信 (AD:publicDelegates) は、ユーザーのメールボックスへのアクセス権を持つ受信者の DN を代理人として格納します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-419">Send On Behalf Of (AD:publicDelegates) stores the DN of recipients with access to a user’s mailbox as a delegate.</span></span> <span data-ttu-id="c5d49-420">この値は Active Directory に格納され、現在、メールボックスの移行の一部として移動されません。</span><span class="sxs-lookup"><span data-stu-id="c5d49-420">This value is stored in Active Directory and currently does not move as part of the mailbox transition.</span></span> <span data-ttu-id="c5d49-421">移動元メールボックスに publicDelegates が設定されている場合は、実行してターゲット環境で MEU からメールボックスへの変換が完了したら、ターゲット メールボックスの publicDelegates を再サンプリングする必要があります `Set-Mailbox <principle> -GrantSendOnBehalfTo <delegate>` 。</span><span class="sxs-lookup"><span data-stu-id="c5d49-421">If the source mailbox has publicDelegates set, you will need to restamp the publicDelegates on the target Mailbox once the MEU to Mailbox conversion completes in the target environment by running `Set-Mailbox <principle> -GrantSendOnBehalfTo <delegate>`.</span></span> 
 
- <span data-ttu-id="c5d49-422">メールボックスに格納されているメールボックスのアクセス許可は、プリンシパルと代理人の両方がターゲット システムに移動すると、メールボックスと共に移動します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-422">Mailbox Permissions that are stored in the mailbox will move with the mailbox when both the principal and the delegate are moved to the target system.</span></span> <span data-ttu-id="c5d49-423">たとえば、ユーザーは、TestUser_7内のメールボックス に FullAccess TestUser_8付与SourceCompany.onmicrosoft.com。</span><span class="sxs-lookup"><span data-stu-id="c5d49-423">For example, the user TestUser_7 is granted FullAccess to the mailbox TestUser_8 in the tenant SourceCompany.onmicrosoft.com.</span></span> <span data-ttu-id="c5d49-424">メールボックスの移動が完了してメールボックスTargetCompany.onmicrosoft.com、ターゲット ディレクトリに同じアクセス許可が設定されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-424">After the mailbox move completes to TargetCompany.onmicrosoft.com, the same permissions are set up in the target directory.</span></span> <span data-ttu-id="c5d49-425">ソース テナントとターゲット テナントの両方TestUser_7に *Get-MailboxPermission* を使用する例を以下に示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-425">Examples using *Get-MailboxPermission* for TestUser_7 in both source and target tenants are shown below.</span></span> <span data-ttu-id="c5d49-426">Exchange コマンドレットには、ソースとターゲットのプレフィックスが付いて表示されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-426">Exchange cmdlets are prefixed with source and target accordingly.</span></span> 
 
<span data-ttu-id="c5d49-427">移動前のメールボックスアクセス許可の出力の例を次に示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-427">Here's an example of the output of the mailbox permission before a move.</span></span> 
...
```
<span data-ttu-id="c5d49-428">移動後のメールボックスアクセス許可の出力の例を次に示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-428">Here's an example of the output of the mailbox permission after the move.</span></span> 
...
> [!Note]
> <span data-ttu-id="c5d49-429">テナント間のメールボックスと予定表のアクセス許可はサポートされていません。</span><span class="sxs-lookup"><span data-stu-id="c5d49-429">Cross-tenant mailbox and calendar permissions are NOT supported.</span></span> <span data-ttu-id="c5d49-430">プリンシパルと代理人を統合移動バッチに整理して、接続されたメールボックスが移行元テナントから同時に移行される必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-430">You must organize principals and delegates into consolidated move batches so that these connected mailboxes are transitioned at the same time from the source tenant.</span></span> 

<span data-ttu-id="c5d49-431">**移行を有効にするには、どの X500 プロキシをターゲットの MailUser プロキシ アドレスに追加する必要がありますか。**</span><span class="sxs-lookup"><span data-stu-id="c5d49-431">**What X500 proxy should be added to the target MailUser proxy addresses to enable migration?**</span></span>  

<span data-ttu-id="c5d49-432">テナント間メールボックスの移行では、移動元メールボックス オブジェクトの LegacyExchangeDN 値が、移動先の MailUser オブジェクトの x500 電子メール アドレスとしてスタンプされる必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-432">The cross-tenant mailbox migration requires that the LegacyExchangeDN value of the source mailbox object to be stamped as an x500 email address on the target MailUser object.</span></span>  

<span data-ttu-id="c5d49-433">例:</span><span class="sxs-lookup"><span data-stu-id="c5d49-433">Example:</span></span>  
```powershell
...
> [!Note]  
> <span data-ttu-id="c5d49-434">この X500 プロキシに加えて、ソース内のメールボックスからターゲット内のメールボックスに、すべての X500 プロキシをコピーする必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-434">In addition to this X500 proxy, you will need to copy all X500 proxies from the mailbox in the source to the mailbox in the target.</span></span>  

<span data-ttu-id="c5d49-435">**Azure Key Vault は必要ですか。また、いつトランザクションが行われたか。**</span><span class="sxs-lookup"><span data-stu-id="c5d49-435">**Is Azure Key Vault required and when are transactions made?**</span></span>  

<span data-ttu-id="c5d49-436">はい、移行を承認する証明書を保存するために Key Vault を使用するには、Azure サブスクリプションが必要です。</span><span class="sxs-lookup"><span data-stu-id="c5d49-436">Yes, an Azure subscription is required to use Key Vault to store the certificate to authorize migration.</span></span> <span data-ttu-id="c5d49-437">ユーザー名 & パスワードを使用してソースに対する認証を行うオンボード移行とは異なり、テナント間メールボックスの移行では OAuth とこの証明書をシークレット/資格情報として使用します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-437">Unlike onboarding migrations which use username & password to authenticate to the source, cross-tenant mailbox migrations use OAuth and this certificate as the secret/credential.</span></span> <span data-ttu-id="c5d49-438">Key Vault へのアクセスは、移行の開始時と終了時に 1 回、増分同期時に 24 時間ごとに 1 回アクセスされるのと同様に、すべてのメールボックスの移行を通じて維持する必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-438">Access to the Key Vault must be maintained throughout all mailbox migrations as it is accessed once at the beginning and once end of migration, as well as once every 24 hours during incremental sync times.</span></span> <span data-ttu-id="c5d49-439">ARK のコストの詳細については、こちらを参照 [してください]( https://azure.microsoft.com/en-us/pricing/details/key-vault/)。</span><span class="sxs-lookup"><span data-stu-id="c5d49-439">You can review AKV costing details [here]( https://azure.microsoft.com/en-us/pricing/details/key-vault/).</span></span>  

<span data-ttu-id="c5d49-440">**バッチ処理に関する推奨事項はありますか?**</span><span class="sxs-lookup"><span data-stu-id="c5d49-440">**Do you have any recommendations for batches?**</span></span>  

<span data-ttu-id="c5d49-441">バッチあたりのメールボックス数は 2000 以下です。</span><span class="sxs-lookup"><span data-stu-id="c5d49-441">Do not exceed 2000 mailboxes per batch.</span></span> <span data-ttu-id="c5d49-442">同期中にエンド ユーザーに影響が出ない場合は、カットオーバー日の 2 週間前にバッチを送信することを強く推奨します。50,000 を超えるメールボックスの数量に関するガイダンスが必要な場合は、次のページでエンジニアリング フィードバック配布リストcrosstenantmigrationpreview@service.microsoft.com。</span><span class="sxs-lookup"><span data-stu-id="c5d49-442">We strongly recommend submitting batches two weeks prior to the cut-over date as there is no impact to the end users during sync. If you need guidance for mailboxes quantities over 50,000 you can reach out to the Engineering Feedback Distribution List at crosstenantmigrationpreview@service.microsoft.com.</span></span>

<span data-ttu-id="c5d49-443">**顧客キーでサービスの暗号化を使用する場合**</span><span class="sxs-lookup"><span data-stu-id="c5d49-443">**What if I use Service encryption with Customer Key?**</span></span>

<span data-ttu-id="c5d49-444">メールボックスは移動前に復号化されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-444">The mailbox will be decrypted prior to moving.</span></span> <span data-ttu-id="c5d49-445">引き続き必要な場合は、ターゲット テナントで顧客キーが構成されていることを確認します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-445">Ensure Customer Key is configured in the target tenant if it is still required.</span></span> <span data-ttu-id="c5d49-446">詳細 [については、](https://docs.microsoft.com/microsoft-365/compliance/customer-key-overview) こちらを参照してください。</span><span class="sxs-lookup"><span data-stu-id="c5d49-446">See [here](https://docs.microsoft.com/microsoft-365/compliance/customer-key-overview) for more information.</span></span>  

<span data-ttu-id="c5d49-447">**推定移行時間は何時ですか?**</span><span class="sxs-lookup"><span data-stu-id="c5d49-447">**What is the estimated migration time?**</span></span>

<span data-ttu-id="c5d49-448">移行を計画する際に役立つ、次[](https://docs.microsoft.com/exchange/mailbox-migration/office-365-migration-best-practices#estimated-migration-times)の表に、一括メールボックスの移行または個々の移行が完了すると予想される場合のガイドラインを示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-448">To help you plan your migration, the table present [here](https://docs.microsoft.com/exchange/mailbox-migration/office-365-migration-best-practices#estimated-migration-times) shows the guidelines about when to expect bulk mailbox migrations or individual migrations to complete.</span></span> <span data-ttu-id="c5d49-449">これらの見積もりは、以前の顧客移行のデータ分析に基づいて行います。</span><span class="sxs-lookup"><span data-stu-id="c5d49-449">These estimates are based on a data analysis of previous customer migrations.</span></span> <span data-ttu-id="c5d49-450">どの環境も一意なので、移行の正確な速度は異なる場合があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-450">Because every environment is unique, your exact migration velocity may vary.</span></span>  

<span data-ttu-id="c5d49-451">この機能は現在プレビュー段階であり、SLA および適用可能なサービス レベルは、この機能のプレビュー状態中のパフォーマンスや可用性の問題には適用されません。</span><span class="sxs-lookup"><span data-stu-id="c5d49-451">Do remember that this feature is currently in preview and the SLA and any applicable Service Levels do not apply to any performance or availability issues during the preview status of this feature.</span></span>

## <a name="known-issues"></a><span data-ttu-id="c5d49-452">既知の問題</span><span class="sxs-lookup"><span data-stu-id="c5d49-452">Known issues</span></span>  

-  <span data-ttu-id="c5d49-453">**問題: 自動拡張アーカイブを移行できません。**</span><span class="sxs-lookup"><span data-stu-id="c5d49-453">**Issue: Auto Expanded archives cannot be migrated.**</span></span> <span data-ttu-id="c5d49-454">テナント間の移行機能は、特定のユーザーのプライマリ メールボックスとアーカイブ メールボックスの移行をサポートします。</span><span class="sxs-lookup"><span data-stu-id="c5d49-454">The cross-tenant migration feature support migrations of the primary mailbox and archive mailbox for a specific user.</span></span> <span data-ttu-id="c5d49-455">ただし、ソース内のユーザーが自動拡張アーカイブ (複数のアーカイブ メールボックスを意味する) を持つ場合、この機能は追加のアーカイブを移行できません。</span><span class="sxs-lookup"><span data-stu-id="c5d49-455">If the user in the source however has an auto expanded archive – meaning more than one archive mailbox, the feature is unable to migrate the additional archives.</span></span>

- <span data-ttu-id="c5d49-456">**問題: 所有されていない smtp proxyAddress ブロック MRS を持つ Cloud MailUsers がバックグラウンドで移動します。**</span><span class="sxs-lookup"><span data-stu-id="c5d49-456">**Issue: Cloud MailUsers with non-owned smtp proxyAddress block MRS moves background.**</span></span> <span data-ttu-id="c5d49-457">ターゲット テナントの MailUser オブジェクトを作成する場合は、すべての SMTP プロキシ アドレスがターゲット テナント組織に属している必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-457">When creating target tenant MailUser objects, you must ensure that all SMTP proxy addresses belong to the target tenant organization.</span></span> <span data-ttu-id="c5d49-458">ローカル テナントに属さないターゲット メール ユーザーに SMTP proxyAddress が存在する場合、MailUser から Mailbox への変換は防止されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-458">If an SMTP proxyAddress exists on the target mail user that does not belong to the local tenant, the conversion of the MailUser to Mailbox is prevented.</span></span> <span data-ttu-id="c5d49-459">これは、メールボックス オブジェクトが、テナントが権限を持つドメイン (テナントによって要求されたドメイン) からのみメールを送信できるという保証のためです。</span><span class="sxs-lookup"><span data-stu-id="c5d49-459">This is due to our assurance that mailbox objects can only send mail from domains for which the tenant is authoritative (domains claimed by the tenant):</span></span> 

   - <span data-ttu-id="c5d49-460">Azure AD Connect を使用してオンプレミスからユーザーを同期する場合は、メールボックスが存在するソース テナント (laran@contoso.onmicrosoft.com) を指す ExternalEmailAddress を使用してオンプレミスの MailUser オブジェクトをプロビジョニングし、PrimarySMTPAddress をターゲット テナント (Lara.Newton@northwind.com) に存在するドメインとしてスタンプします。</span><span class="sxs-lookup"><span data-stu-id="c5d49-460">When you sync users from on-premises using Azure AD Connect, you provision on-premises MailUser objects with ExternalEmailAddress pointing to the source tenant where the mailbox exists (laran@contoso.onmicrosoft.com) and you stamp the PrimarySMTPAddress as a domain that resides in the target tenant (Lara.Newton@northwind.com).</span></span> <span data-ttu-id="c5d49-461">これらの値はテナントに同期され、適切なメール ユーザーがプロビジョニングされ、移行の準備が整います。</span><span class="sxs-lookup"><span data-stu-id="c5d49-461">These values sync down to the tenant and an appropriate mail user is provisioned and ready for migration.</span></span> <span data-ttu-id="c5d49-462">オブジェクトの例を次に示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-462">An example object is shown here.</span></span>
     ```powershell
   ...
   > [!Note]
   > <span data-ttu-id="c5d49-463">電子 *contoso.onmicrosoft.com\*\*アドレスが* EmailAddresses / proxyAddresses 配列に存在しない。</span><span class="sxs-lookup"><span data-stu-id="c5d49-463">The *contoso.onmicrosoft.com* address is *not* present in the EmailAddresses / proxyAddresses array.</span></span>

- <span data-ttu-id="c5d49-464">**問題: "外部" プライマリ SMTP アドレスを持つ MailUser オブジェクトが、"内部" の会社の要求ドメインに変更またはリセットされる**</span><span class="sxs-lookup"><span data-stu-id="c5d49-464">**Issue: MailUser objects with “external” primary SMTP addresses are modified / reset to “internal” company claimed domains**</span></span>

   <span data-ttu-id="c5d49-465">MailUser オブジェクトは、ローカル以外のメールボックスへのポインターです。</span><span class="sxs-lookup"><span data-stu-id="c5d49-465">MailUser objects are pointers to non-local mailboxes.</span></span> <span data-ttu-id="c5d49-466">テナント間のメールボックス移行の場合は、MailUser オブジェクトを使用して、移動元メールボックス (ターゲット組織の観点から) またはターゲット メールボックス (移行元組織の観点から) を表します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-466">In the case for cross-tenant mailbox migrations, we use MailUser objects to represent either the source mailbox (from the target organization’s perspective) or target mailbox (from the source organization’s perspective).</span></span> <span data-ttu-id="c5d49-467">MailUsers には、ディレクトリ内のメールボックス ユーザーの表示される SMTP アドレスを表す、実際のメールボックス (ProxyTest@fabrikam.onmicrosoft.com) の smtp アドレスと primarySMTP アドレスをポイントする ExternalEmailAddress (targetAddress) があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-467">The MailUsers will have an ExternalEmailAddress (targetAddress) that points to the smtp address of the actual mailbox (ProxyTest@fabrikam.onmicrosoft.com) and primarySMTP address that represents the displayed SMTP address of the mailbox user in the directory.</span></span> <span data-ttu-id="c5d49-468">一部の組織では、プライマリ SMTP アドレスを外部 SMTP アドレスとして表示し、ローカル テナントによって所有/検証されるアドレス (fabrikam.com など、 contoso.com など) として表示しない場合があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-468">Some organizations choose to display the primary SMTP address as an external SMTP address, not as an address owned/verified by the local tenant (such as fabrikam.com rather than as contoso.com).</span></span>  <span data-ttu-id="c5d49-469">ただし、ライセンス操作によって Exchange サービス プラン オブジェクトが MailUser に適用されると、プライマリ SMTP アドレスが変更され、ローカル組織 (contoso.com) によって確認されたドメインとして表示されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-469">However, once an Exchange service plan object is applied to the MailUser via licensing operations, the primary SMTP address is modified to show as a domain verified by the local organization (contoso.com).</span></span> <span data-ttu-id="c5d49-470">次の 2 つの潜在的な理由があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-470">There are two potential reasons:</span></span>
   
   - <span data-ttu-id="c5d49-471">Exchange サービス プランが MailUser に適用されると、Azure AD プロセスはプロキシ スクラブの適用を開始し、ローカル組織が別のテナントからメールを送信、スプーフィング、またはメールを送信できないか確認します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-471">When any Exchange service plan is applied to a MailUser, the Azure AD process starts to enforce proxy scrubbing to ensure that the local organization is not able to send mail out, spoof, or mail from another tenant.</span></span> <span data-ttu-id="c5d49-472">これらのサービス プランを持つ受信者オブジェクトの SMTP アドレスは、そのアドレスがローカル組織によって確認されていない場合に削除されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-472">Any SMTP address on a recipient object with these service plans will be removed if the address is not verified by the local organization.</span></span> <span data-ttu-id="c5d49-473">例の場合と同様に、Fabikam.com ドメインは contoso.onmicrosoft.com テナントによって検証されないので、スクラブによってそのドメインfabrikam.comされます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-473">As is the case in the example, the Fabikam.com domain is NOT verified by the contoso.onmicrosoft.com tenant, so the scrubbing removes that fabrikam.com domain.</span></span> <span data-ttu-id="c5d49-474">移行前または移行後にこれらの外部ドメインを MailUser に保持する場合は、移行が完了した後、または移動前にライセンスを削除して、ユーザーに予期される外部ブランド化が適用されるかどうかを確認するために、移行プロセスを変更する必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-474">If you wish to persist these external domain on MailUser, either before the migration or after migration, you need to alter your migration processes to strip licenses after the move completes or before the move to ensure that the users have the expected external branding applied.</span></span> <span data-ttu-id="c5d49-475">メール サービスに影響を与えずに、メールボックス オブジェクトに適切なライセンスが与えされていることを確認する必要があります。</span><span class="sxs-lookup"><span data-stu-id="c5d49-475">You will need to ensure that the mailbox object is properly licensed to not affect mail service.</span></span><br/><br/><span data-ttu-id="c5d49-476">テナント内の MailUser のサービス プランを削除するスクリプトContoso.onmicrosoft.com次に示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-476">An example script to remove the service plans on a MailUser in the Contoso.onmicrosoft.com tenant is shown here.</span></span>
    ...
    ```
       ...
       <span data-ttu-id="c5d49-477">割り当てられた ServicePlans のセットの結果を次に示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-477">Results in the set of ServicePlans assigned are shown here.</span></span>
    ...
    ```
       ...
       <span data-ttu-id="c5d49-478">ユーザーの PrimarySMTPAddress がスクラブされなくなりました。</span><span class="sxs-lookup"><span data-stu-id="c5d49-478">The user’s PrimarySMTPAddress is no longer scrubbed.</span></span> <span data-ttu-id="c5d49-479">ドメインfabrikam.comは、contoso.onmicrosoft.com テナントによって所有されていないので、ディレクトリに表示されるプライマリ SMTP アドレスとして保持されます。</span><span class="sxs-lookup"><span data-stu-id="c5d49-479">The fabrikam.com domain is not owned by the contoso.onmicrosoft.com tenant and will persist as the primary SMTP address shown in the directory.</span></span>

       <span data-ttu-id="c5d49-480">次に例を示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-480">Here is an example.</span></span>
   ...
   - <span data-ttu-id="c5d49-481">msExchRemoteRecipientType が 8 (DeprovisionMailbox) に設定されている場合、ターゲット テナントに移行されるオンプレミスの MailUser の場合、Azure のプロキシ スクラブ ロジックは、所有されていないドメインを削除し、primarySMTP を所有ドメインにリセットします。</span><span class="sxs-lookup"><span data-stu-id="c5d49-481">When msExchRemoteRecipientType is set to 8 (DeprovisionMailbox), for on-premises MailUsers that are migrated to the target tenant, the proxy scrubbing logic in Azure will remove nonowned domains and reset the primarySMTP to an owned domain.</span></span> <span data-ttu-id="c5d49-482">オンプレミスの MailUser で msExchRemoteRecipientType をクリアすると、プロキシ スクラブ ロジックは適用されなくなりました。</span><span class="sxs-lookup"><span data-stu-id="c5d49-482">By clearing msExchRemoteRecipientType in the on-premises MailUser, the proxy scrub logic no longer applies.</span></span> <br/><br><span data-ttu-id="c5d49-483">以下に、Exchange Online を含む可能性があるサービス プランの完全なセットを示します。</span><span class="sxs-lookup"><span data-stu-id="c5d49-483">Below is the full set of possible Service Plans that include Exchange Online.</span></span>

   | <span data-ttu-id="c5d49-484">名前</span><span class="sxs-lookup"><span data-stu-id="c5d49-484">Name</span></span>                                              |
   |---------------------------------------------------|
   | <span data-ttu-id="c5d49-485">Advanced eDiscovery Storage (500GB)</span><span class="sxs-lookup"><span data-stu-id="c5d49-485">Advanced eDiscovery Storage (500GB)</span></span>               |
   | <span data-ttu-id="c5d49-486">顧客ロックボックス</span><span class="sxs-lookup"><span data-stu-id="c5d49-486">Customer Lockbox</span></span>                                  |
   | <span data-ttu-id="c5d49-487">データ損失防止</span><span class="sxs-lookup"><span data-stu-id="c5d49-487">Data Loss Prevention</span></span>                              |
   | <span data-ttu-id="c5d49-488">Exchange Enterprise CAL サービス (EOP、DLP)</span><span class="sxs-lookup"><span data-stu-id="c5d49-488">Exchange Enterprise CAL Services (EOP, DLP)</span></span>       |
   | <span data-ttu-id="c5d49-489">Exchange Essentials</span><span class="sxs-lookup"><span data-stu-id="c5d49-489">Exchange Essentials</span></span>                               |
   | <span data-ttu-id="c5d49-490">Exchange Foundation</span><span class="sxs-lookup"><span data-stu-id="c5d49-490">Exchange Foundation</span></span>                               |
   | <span data-ttu-id="c5d49-491">Exchange Online (P1)</span><span class="sxs-lookup"><span data-stu-id="c5d49-491">Exchange Online (P1)</span></span>                              |
   | <span data-ttu-id="c5d49-492">Exchange Online (プラン 1)</span><span class="sxs-lookup"><span data-stu-id="c5d49-492">Exchange Online (Plan 1)</span></span>                          |
   | <span data-ttu-id="c5d49-493">Exchange Online (プラン 2)</span><span class="sxs-lookup"><span data-stu-id="c5d49-493">Exchange Online (Plan 2)</span></span>                          |
   | <span data-ttu-id="c5d49-494">Exchange Online 用の Exchange Online Archiving</span><span class="sxs-lookup"><span data-stu-id="c5d49-494">Exchange Online Archiving for Exchange Online</span></span>     |
   | <span data-ttu-id="c5d49-495">Exchange Server 用の Exchange Online Archiving</span><span class="sxs-lookup"><span data-stu-id="c5d49-495">Exchange Online Archiving for Exchange Server</span></span>     |
   | <span data-ttu-id="c5d49-496">Exchange Online 非アクティブユーザー アドオン</span><span class="sxs-lookup"><span data-stu-id="c5d49-496">Exchange Online Inactive User Add-on</span></span>              |
   | <span data-ttu-id="c5d49-497">Exchange Online Kiosk</span><span class="sxs-lookup"><span data-stu-id="c5d49-497">Exchange Online Kiosk</span></span>                             |
   | <span data-ttu-id="c5d49-498">Exchange Online Multi-Geo</span><span class="sxs-lookup"><span data-stu-id="c5d49-498">Exchange Online Multi-Geo</span></span>                         |
   | <span data-ttu-id="c5d49-499">Exchange Online プラン 1</span><span class="sxs-lookup"><span data-stu-id="c5d49-499">Exchange Online Plan 1</span></span>                            |
   | <span data-ttu-id="c5d49-500">Exchange Online POP</span><span class="sxs-lookup"><span data-stu-id="c5d49-500">Exchange Online POP</span></span>                               |
   | <span data-ttu-id="c5d49-501">Exchange Online Protection</span><span class="sxs-lookup"><span data-stu-id="c5d49-501">Exchange Online Protection</span></span>                        |
   | <span data-ttu-id="c5d49-502">情報バリア</span><span class="sxs-lookup"><span data-stu-id="c5d49-502">Information Barriers</span></span>                              |
   | <span data-ttu-id="c5d49-503">Information Protection for Office 365 - Premium</span><span class="sxs-lookup"><span data-stu-id="c5d49-503">Information Protection for Office 365 - Premium</span></span>   |
   | <span data-ttu-id="c5d49-504">Information Protection for Office 365 - Standard</span><span class="sxs-lookup"><span data-stu-id="c5d49-504">Information Protection for Office 365 - Standard</span></span>  |
   | <span data-ttu-id="c5d49-505">MyAnalytics による分析情報</span><span class="sxs-lookup"><span data-stu-id="c5d49-505">Insights by MyAnalytics</span></span>                           |
   | <span data-ttu-id="c5d49-506">Microsoft 365 Advanced Auditing</span><span class="sxs-lookup"><span data-stu-id="c5d49-506">Microsoft 365 Advanced Auditing</span></span>                   |
   | <span data-ttu-id="c5d49-507">Microsoft Bookings</span><span class="sxs-lookup"><span data-stu-id="c5d49-507">Microsoft Bookings</span></span>                                |
   | <span data-ttu-id="c5d49-508">Microsoft Business Center</span><span class="sxs-lookup"><span data-stu-id="c5d49-508">Microsoft Business Center</span></span>                         |
   | <span data-ttu-id="c5d49-509">Microsoft MyAnalytics (フル機能)</span><span class="sxs-lookup"><span data-stu-id="c5d49-509">Microsoft MyAnalytics (Full)</span></span>                      |
   | <span data-ttu-id="c5d49-510">Office 365 Advanced eDiscovery</span><span class="sxs-lookup"><span data-stu-id="c5d49-510">Office 365 Advanced eDiscovery</span></span>                    |
   | <span data-ttu-id="c5d49-511">Microsoft Defender for Office 365 (プラン 1)</span><span class="sxs-lookup"><span data-stu-id="c5d49-511">Microsoft Defender for Office 365 (Plan 1)</span></span>    |
   | <span data-ttu-id="c5d49-512">Microsoft Defender for Office 365 (プラン 2)</span><span class="sxs-lookup"><span data-stu-id="c5d49-512">Microsoft Defender for Office 365 (Plan 2)</span></span>    |
   | <span data-ttu-id="c5d49-513">Office 365 Privileged Access Management</span><span class="sxs-lookup"><span data-stu-id="c5d49-513">Office 365 Privileged Access Management</span></span>           |
   | <span data-ttu-id="c5d49-514">Office 365 でのプレミアム暗号化</span><span class="sxs-lookup"><span data-stu-id="c5d49-514">Premium Encryption in Office 365</span></span>                  |
