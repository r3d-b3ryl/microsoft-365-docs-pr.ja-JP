...
Azure AD では、事前に構成された条件付きアクセスポリシーを使用して管理者アクセスの保護を開始する簡単な方法を提供します。 Azure AD で、**条件付きアクセス**に移動して、このポリシーを探します。**ベースラインポリシー: 管理者に MFA を必須**にします。 このポリシーを選択し、[**すぐにポリシーを使用する**] を選択します。 
