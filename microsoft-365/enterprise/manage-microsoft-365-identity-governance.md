...
Azure ADアクセス レビューを使用すると、ユーザーのアクセス権を確認して、適切なユーザーのみがアクセスを継続できることを確認できます。 例:
...
## <a name="set-up-azure-ad-entitlement-management"></a>エンタイトルメント管理Azure AD設定する

Wiht Azure ADエンタイトルメント管理では、アクセス要求ワークフロー、アクセス割り当て、レビュー、有効期限を自動化することで、ID とアクセスのライフサイクルを大規模に管理できます。
...
Azure ADエンタイトルメント管理は、グループ、アプリケーション、およびSharePoint サイトへのアクセスをより効率的に管理するのに役立ちます。内部および外部のユーザー向けです。
 
詳細については、[Azure ADエンタイトルメント管理の概要に関するページ](/azure/active-directory/governance/entitlement-management-overview)を参照してください。