...
- データの取得/キャッシュ
  - Web パーツが、表示用にデータをフェッチするために特別なサーバー呼び出しを使用している場合は、それらのサーバー Api が高速であること、またはクライアント側のキャッシュを実装していることを確認してください (たとえば、 _Localstorage_ の使用や大規模なセットに対する _indexeddb_ の実装)。
