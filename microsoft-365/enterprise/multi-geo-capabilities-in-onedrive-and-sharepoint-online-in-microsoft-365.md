...
OneDriveとSharePoint Online の複数地域機能を使用すると、SharePoint チーム サイトやMicrosoft 365グループ メールボックスなどの共有リソースを、指定された地理的な場所に保存して保存できます。
...
Exchange、OneDrive、SharePoint、Teams以外のMicrosoft 365 サービスは Multi-geo ではありません。 ただし、これらのサービスによって作成されるMicrosoft 365 グループは、作成者の PDL とそのExchange グループ メールボックスで構成されます。SharePoint サイトは対応する geo でプロビジョニングされます。 
