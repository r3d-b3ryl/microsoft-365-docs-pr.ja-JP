...
ms.locfileid: "60177137"
---
# <a name="navigation-options-for-sharepoint-online"></a>SharePoint Online のナビゲーション オプション

この記事では、SharePoint Publishing が有効化されている SharePoint Online サイトのナビゲーション オプションについて説明します。 ナビゲーションの選択と構成は、SharePoint Online のサイトのパフォーマンスとスケーラビリティに大きく影響します。 SharePoint発行サイト テンプレートは、一元的なポータルに必要な場合にのみ使用し、発行機能は特定のサイトでのみ有効にする必要があります。また、誤って使用するとパフォーマンスに影響を与える可能性がある場合にのみ、絶対に必要な場合にのみ使用してください。

>[!NOTE]
>メガ メニュー、カスケード ナビゲーションSharePointハブ ナビゲーションなど、最新のナビゲーション オプションを使用している場合、この記事はサイトには適用されません。 最新SharePointサイト アーキテクチャは、よりフラット化されたサイト階層とハブ アンド スポーク モデルを活用します。 これにより、発行機能の使用を必要としない多くのシナリオをSharePointできます。
...
>[!NOTE]
>全体的なサイト構造をフラットで階層的でない構造に簡素化することで、モダン SharePoint サイトと同じ原則を適用すると、パフォーマンスが向上し、最新の SharePoint サイトへの移行が簡素化されます。 つまり、何百ものサイト (サブ web) を持つ単一のサイト コレクションを持つ代わりに、サブサイト (サブ web) が非常に少ない多数のサイト コレクションを持つ方が良い方法です。

## <a name="analyzing-navigation-performance-in-sharepoint-online"></a>オンラインでのナビゲーションのパフォーマンスSharePointする

[[ページ診断]](./page-diagnostics-for-spo.md) SharePoint ツールは、Microsoft Edge および Chrome ブラウザーのブラウザー拡張機能で、SharePoint Online モダン ポータルと従来の発行サイト ページの両方を分析します。 このツールは、SharePointオンラインでのみ機能し、システム ページSharePoint使用できません。
...
**SPRequestDuration は** 特に、ページの処理にSharePoint時間です。 ナビゲーションの重いナビゲーション (ナビゲーションのページなど)、複雑なサイト階層、その他の構成オプションやトポロジ オプションはすべて、より長い期間に大きく貢献します。
...
管理ナビゲーションの詳細については、docs.microsoft.com に関する記事がいくつか掲載されています。 たとえば、「サーバーの[管理ナビゲーションの概要」をSharePointします](/sharepoint/administration/overview-of-managed-navigation)。
...
>[!NOTE]
>カスタム JavaScript を使用する場合は、パブリック CDNが有効で、ファイルが特定の場所CDNしてください。
