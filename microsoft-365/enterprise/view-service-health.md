...
[Microsoft 365 管理センター](https://go.microsoft.com/fwlink/p/?linkid=2024339)の **[サービス正常性 ]** ページでは、Office on the web,、Yammer、Microsoft Dynamics CRM、モバイル デバイス管理クラウド サービスなどの Microsoft サービスの正常性を表示できます。クラウド サービスの問題が発生している場合は、サポートに連絡、またはトラブルシューティングを行う前に、サービス正常性を確認することで、その問題が解決に向けて取り組み中の既知の問題であるかどうかを判断できます。
...
ダッシュボードに表示されるサービスのビューをカスタマイズするには、**[ユーザー設定]** > **[カスタム ビュー]** の順に選択し、サービス正常性ダッシュボードの表示から除外したいサービスのチェックボックスをオフにします。監視するサービスごとにチェックボックスがオンになっていることを確認します。
...
機械翻訳を利用して、指定された優先言語でメッセージを自動表示します。言語の設定方法については、「[サービス正常性ダッシュボードの言語変換](lang-service-health.md)」を参照してください。
...
> [!TIP]
> 計画済みメンテナンスのイベントはサービス正常性には表示されません。**メッセージ センター** で最新の情報を確認することで、計画済みメンテナンスのイベントを追跡できます。[変更の予定] として分類されたメッセージをフィルター処理で絞り込むと、変更が発生する時期、その影響、およびそのための準備方法を確認できます。詳細については、「[Microsoft 365 のメッセージ センター](https://support.office.com/article/38fb3333-bfcc-4340-a37b-deda509c2093)」を参照してください。
...
サービス正常性を使用すると、現在の正常性の状態を確認し、過去 30 日間にテナントに影響を与えたサービスのアドバイザリとインシデントの履歴を表示することができます。すべてのサービスの過去の正常性を表示するには、**[履歴]** ビューを選択します。
