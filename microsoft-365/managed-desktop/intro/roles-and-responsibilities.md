...
セキュリティの監視 | Microsoft はエンドポイントの Microsoft Defender を使用して、microsoft の管理されたデスクトップデバイスを監視します。 Microsoft Managed デスクトップセキュリティ操作センター (SOC) が脅威を検出した場合は、お客様に通知し、デバイスを分離して、問題をリモートで修正します。 詳細については、「 [Security](../service-description/security.md)」を参照してください。
