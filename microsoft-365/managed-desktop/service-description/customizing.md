...
例外は、Microsoft マネージドデスクトップ基本構成に追加または変更されたものです。例としては、USB ポート構成から新しいセキュリティエージェントを展開する場合があります。 さまざまな例外は、次のようにグループ化します。
...
-    型: excmption のカテゴリはどれですか? (前の表を参照)
-   要件: の特定のビジネス要件は何ですか。
...
要求された例外が承認および展開された後、最初の場所で変更を承認したときに明らかになっていない重要な条件に違反する問題を検出する可能性があります。 このような状況では、の承認を取り消す必要がある場合があります。
...
- **最初の通知から90日後:** Microsoft マネージドデスクトップサービスレベルアグリーメントは、失効したデバイスには適用されなくなりました。 いつでも、意思決定をチャレンジして、アップグレード、構成の変更、ソフトウェアの変更など、考慮すべき追加情報を提供することができます。 
