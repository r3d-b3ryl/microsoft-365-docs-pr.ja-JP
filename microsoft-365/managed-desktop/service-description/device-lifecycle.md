...
Microsoft マネージドデスクトップの利点エンドユーザーは、最高のパフォーマンス、信頼性、設計、およびセキュリティ機能を提供するデバイスを常に使用することを保証します (Windows Hello などの機能のサポートなど)。 これを実現するために、Microsoft マネージドデスクトップは、継続的に更新される[承認済みデバイス](device-list.md)の短いカタログを保持します。 
...
## <a name="product-lifecycle"></a>製品ライフサイクル
...
製品は最大24か月間カタログに保持されますが、*デバイス*はそれぞれの登録日に基づいて3年間管理されたままになります。 実質的に、各製品には3つの重要な日付がありますが、各デバイスには1つしかありません。 製品については、これらの3つの日付すべてが*承認日*に基づいて計算されるため、これらの日付を承認時に公開するので、常に製品のライフサイクル全体に対して適切に検討し、計画することができます。

次の表に、理論上の製品の日付の例を示します。


|製品  |承認日  |プライマリ可用性の終了  |アドオンの終了  |
|---------|---------|---------|---------|
...
この期間は効率的に "登録を開く" と考えることができます。そのため、Microsoft マネージドデスクトップの価値を最大にするために、調達モデルと選択した製品をこの期間内に収まるようにする必要があります。 簡単な例として、お客様が2年間の製品を使用して、第1の主要な可用性を実現することを避ける必要があります。これらのデバイスのほとんどは、Microsoft の管理されたデスクトップ管理 (「猶予」をご覧ください。 [期間](#product-grace-period)の詳細を参照してください)。  
...
Microsoft マネージドデスクトップチームは、調達サイクルが長く、長期予算に関連して計画されていることを認識しています。 常にデバイスの作成の状態を把握していることを確認するために、管理対象のすべてのデバイス、今後の退職日、およびコンプライアンスを示すステータスを一覧表示する[web サイト](https://aka.ms/mmdportal)を提供しています。 これは、デバイスの有効期限に関する最新情報を常に持っており、任意の調達計画サイクルでレポートを利用できることを意味します。 


また、次の自動化された操作を実行して、新しいデバイスが時間に展開されるようにします。


|タイムライン  |Action  |
|---------|---------|
|T-90     | デバイスインベントリ web サイトに黄色のマーカーが付いた、このデバイスに**近い期限切れ**のフラグを付けます。  |
|T-60     | デバイスインベントリ web サイト上で赤いマーカーを使用して、このデバイスに**期限切れ**のフラグを設定します。       |
|T-30     | デバイスが準拠していないことを示すメッセージを管理ポータルに投稿します。       |
|.0     |  管理ポータルを調整して、デバイスが有効期限切れになったことを示すようにして、最初にデバイスリストに着陸するようにしてください。       |
|T + 30     |  新しいデバイスが展開されるまで、管理ポータルの機能は減少します。       |
|T + 60     |  新しいデバイスが展開されるまで、管理ポータルの機能は減少します。       |
|T + 90     |  管理からデバイスを削除します。 この時点では、デバイスは自分の責任のみがあり、セキュリティで保護されていないことや最新の状態であることを考慮する必要はありません。 また、各構成サービスプロバイダーが独自の設定を制御するため、デバイスは不明な状態になります。|
