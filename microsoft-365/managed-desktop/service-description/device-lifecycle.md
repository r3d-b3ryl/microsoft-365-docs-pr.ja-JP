...
Microsoft マネージドデスクトップでは、ユーザーが、最高のパフォーマンス、信頼性、設計、およびセキュリティ機能 (Windows Hello のような機能のサポートなど) を提供するデバイスを常に使用できるという利点があります。 これを実現するために、Microsoft マネージドデスクトップは、継続的に更新される [承認済みデバイス](device-list.md)の短いカタログを保持します。 
