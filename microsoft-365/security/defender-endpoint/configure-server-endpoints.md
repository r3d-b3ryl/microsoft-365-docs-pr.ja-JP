...
1. 次のコマンドを実行して、インストールMicrosoft Defender ウイルス対策確認します。
    ...
    結果が表示される `The specified service doesn't exist as an installed service` 場合は、インストールする必要Microsoft Defender ウイルス対策。 詳細については、「Microsoft Defender ウイルス対策[サーバー Windows」 を参照してください](microsoft-defender-antivirus-on-windows-server.md)。
