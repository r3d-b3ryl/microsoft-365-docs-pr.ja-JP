...
1. 説明: グループでカバーするデバイス プロパティを一覧表示します。 各デバイス プロパティの詳細については、上記 **の「デバイスのプロパティ** 」セクションを参照してください。
2. オプション:
    - PrimaryId
...
1. グループを作成する
    ...
    2. グループ 2: サンプルの未承認の [USB](https://github.com/microsoft/mdatp-devicecontrol/tree/main/Removable%20Storage%20Access%20Control%20Samples) Group.xmlファイルのデバイス プロパティに基づく未承認の USB(ベンダー ID/製品 ID、フレンドリー名 - グループ **65fa649a-a111-4912-9294-fb6337a25038** など)。
...
この機能 (Microsoft エンドポイント マネージャー 管理センター ( ) Devices Configuration profiles Create profile Platform: Windows 10 以降 <https://endpoint.microsoft.com/> \> & \> \> \> Profile: Device Control) はまだ使用できません。
...
値が設定され、ポリシーがグループ ポリシーによって管理されている場合は、クライアント デバイスがポリシー XML パスにアクセスできるかどうかを確認します。
