---
title: Microsoft Defender for Endpoint Device Control リムーバブル Storageアクセス制御、リムーバブル ストレージ メディア
...
|---|---|---|---|
|リムーバブル メディア グループの作成|再利用可能なリムーバブル メディア グループを作成できます|「OMA-URI によるポリシーの展開」[セクションの手順](#deploying-policy-via-oma-uri) 1 | 「グループ ポリシーによるポリシーの展開[」セクションの手順](#deploying-policy-via-group-policy) 1|
|ポリシーの作成|各リムーバブル メディア グループを適用するポリシーを作成できます|「OMA-URI によるポリシーの展開」[セクションの手順](#deploying-policy-via-oma-uri) 2 | 「グループ ポリシーによるポリシーの展開」セクションの手順 [2 と 3](#deploying-policy-via-group-policy) |
|既定の適用|ポリシーがない場合は、既定のアクセス (拒否または許可) をリムーバブル メディアに設定できます|「OMA-URI によるポリシーの[展開」セクションの手順](#deploying-policy-via-oma-uri) 3 | 「グループ ポリシーによるポリシー[の展開」セクションの手順](#deploying-policy-via-group-policy) 4 |
|リムーバブル アクセス制御を有効またはStorageする|[無効にする] を設定すると、このコンピューターのリムーバブル Storageアクセス制御ポリシーが無効にされます。| 「OMA-URI によるポリシーの展開」[セクションの手順](#deploying-policy-via-oma-uri) 4 | 「グループ ポリシーによるポリシー[の展開」セクションの手順](#deploying-policy-via-group-policy) 5 |
|ファイル情報のキャプチャ|書き込みアクセスが発生した場合にファイル情報をキャプチャするポリシーを作成できます| 「[OMA-URI](#deploying-policy-via-oma-uri) によるポリシーの展開」セクションの手順 2 と 5 | 「グループ ポリシーによるポリシーの展開」セクション[の](#deploying-policy-via-group-policy)手順 2 と 6 |

## <a name="prepare-your-endpoints"></a>エンドポイントを準備する

マルウェア対策クライアント Storage **4.18.2103.3** 以降を持つ Windows 10 および Windows 11 デバイスにリムーバブル Storage アクセス制御を展開します。
...
リムーバブル アクセスStorage機能を使用すると、グループ ポリシーを介してユーザーまたはデバイス、または両方にポリシーを適用できます。

### <a name="licensing"></a>ライセンス

リムーバブル アクセス制御の使用を開始するStorage、サブスクリプションを確認[するMicrosoft 365があります](https://www.microsoft.com/microsoft-365/compare-microsoft-365-enterprise-plans?rtc=2)。 リムーバブル アクセス制御にアクセスして使用Storageするには、リムーバブル アクセス制御またはMicrosoft 365 E3必要Microsoft 365 E5。
...
3. ルールとグループ XML ファイルの両方をネットワーク共有フォルダーに保存し、ネットワーク共有フォルダー のパスを [グループ ポリシー] 設定に入 **れる:**\>コンピューター構成管理テンプレート **Windows Components** \>  \> **Microsoft Defender ウイルス対策** \> Device **Control**: 'Define **device control policy groups'** and **'デバイス制御ポリシー ルールを定義する'**。
...
5. リムーバブル アクセス制御を有効Storage無効にする: この値を設定すると、リムーバブル アクセス制御を一時的に無効Storageできます。
...
4. リムーバブル アクセス制御を有効Storage無効にする: この値を設定すると、リムーバブル アクセス制御を一時的に無効Storageできます。
    ...
    **無効** とは、このコンピューターにリムーバブル アクセス制御Storageが実行されていないという意味です。

    :::image type="content" source="images/148609770-3e555883-f26f-45ab-9181-3fb1ff7a38ac.png" alt-text="PowerShell コードStorageの削除可能なアクセス制御":::
...
:::image type="content" source="images/block-removable-storage.png" alt-text="リムーバブル 記憶域のブロック" lightbox="images/block-removable-storage.png":::
...
オンライン のオープン ソースを使用して、または PowerShell を使用して GUID を生成できます 。 PowerShell を使用して [GUID を生成する方法](/powershell/module/microsoft.powershell.utility/new-guid?msclkid=c1398a25a6d911ec9c888875fa1f24f5&view=powershell-7.2)
...
3. グループ ポリシーを使用してポリシーを展開および管理する場合は、PolicyRule と呼ばれる親ノード内の 1 つの XML ファイルに、PolicyRule とすべてのグループをグループと呼ばれる親ノード内の 1 つの XML ファイルに必ず組み合わせてください。Intune で管理する場合は、PolicyRule 1 つの XML ファイル、同じもの、1 つのグループ 1 XML ファイルを保持します。
