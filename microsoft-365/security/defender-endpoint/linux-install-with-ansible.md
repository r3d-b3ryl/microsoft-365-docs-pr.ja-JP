...
---
# <a name="deploy-microsoft-defender-for-endpoint-for-linux-with-ansible"></a>Ansible を使用して Microsoft Defender for Endpoint for Linux を展開する
...
この記事では、Ansible を使用して Defender for Endpoint for Linux を展開する方法について説明します。 展開が成功するには、次のすべてのタスクを完了する必要があります。
...
開始する前に、現在のソフトウェア バージョンの前提条件とシステム要件の説明については、メインの [Defender for Endpoint for Linux](microsoft-defender-endpoint-linux.md) ページを参照してください。
...
- Ansible インストールを作成し、YAML ファイルをアンインストールします。
    ...
    - yum ベースの配布では、次の YAML ファイルを使用します。
...
- [YUM リポジトリの追加または削除](https://docs.ansible.com/ansible/2.3/yum_repository_module.html)

- [yum パッケージ マネージャーを使用してパッケージを管理する](https://docs.ansible.com/ansible/latest/modules/yum_module.html)

- [APT リポジトリの追加と削除](https://docs.ansible.com/ansible/latest/modules/apt_repository_module.html)

- [apt-packages の管理](https://docs.ansible.com/ansible/latest/modules/apt_module.html)
