...
- すべての管理ノードは、関連するファイルに次の形式で `/etc/ansible/hosts` 一覧表示する必要があります。
...
- Defender for Endpoint リポジトリとキーを追加します `add_apt_repo.yml`。
    ...
    > [!NOTE]
    > Oracle Linux と Amazon Linux 2 の場合、[ *ディストリビューション] を "rhel"* に置き換える。
...
次に、タスク ファイルを下または関連する `/etc/ansible/playbooks/` ディレクトリで実行します。
