...
デバイス (エンドポイント[とも呼Microsoft エンドポイント マネージャー](/mem)組織の脅威保護機能を管理するために、Microsoft Intune (Intune) を含むデバイスを使用することをお勧めします。 [詳細については、エンドポイント マネージャー](/mem/endpoint-manager-overview)。
...
1. Azure portal () に移動し[https://portal.azure.com](https://portal.azure.com)、サインインします。

2. [ **Azure Services] で、[** Intune] を **選択します**。

3. 左側のナビゲーション ウィンドウで、[デバイス構成] **を** 選択し、[管理] で **[プロファイル]** **を選択します**。

4. 既存のプロファイルを選択するか、新しいプロファイルを作成します。

> [!TIP]
> サポートが必要な場合 **[「Microsoft Defender for Endpoint with Intune の使用」を参照してください](/mem/intune/protect/advanced-threat-protection#example-of-using-microsoft-defender-atp-with-intune)**。
...
|**ポリシー Microsoft Defender ウイルス対策サービス** プロバイダー (ポリシー CSP) を使用して、ユーザー設定 [を構成する](/windows/client-management/mdm/policy-configuration-service-provider)|[デバイスの制限: Microsoft Defender ウイルス対策](/mem/intune/configuration/device-restrictions-windows-10#microsoft-defender-antivirus) <br/><br/> [ポリシー CSP - Microsoft Defender for Endpoint](/windows/client-management/mdm/policy-csp-defender)|
...
|**暗号化と BitLocker を構成** して、組織で実行されているデバイスの情報を保護Windows|[エンドポイント保護: Windows暗号化](/mem/intune/protect/endpoint-protection-windows-10#windows-encryption) <br/><br/> [11 台Windows 10デバイスWindows BitLocker](/windows/security/information-protection/bitlocker/bitlocker-overview)|
|**資格情報の盗難攻撃から保護するために Microsoft Defender Credential Guard** を構成する|Windows 10、Windows 11、Windows Server 2016、Windows Server 2019、Windows Server 2022 については、「[Endpoint protection: Microsoft Defender Credential Guard](/mem/intune/protect/endpoint-protection-windows-10#microsoft-defender-credential-guard)」を参照してください。 <br/><br/> Windows 7 SP1、Windows Server 2008 R2 SP1、Windows 8.1、および Windows Server 2012 R2 については、「[Pass-the-Hash (PtH) Attacks and Other Credential Theft、Versions 1 および 2](https://www.microsoft.com/download/details.aspx?id=36036)」を参照してください。|
