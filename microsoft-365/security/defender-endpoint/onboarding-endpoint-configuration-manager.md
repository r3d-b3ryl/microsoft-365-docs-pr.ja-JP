...
![クラウド ネイティブ アーキテクチャのイメージ。 ](images/co-management-architecture.png)
*環境アーキテクチャの図*
...
1. コンソールMicrosoft Endpoint Configuration Manager、[アセット] と [**コンプライアンスの概要] \> デバイス コレクション \> に移動します**。
    ...
    ![ウィザード 1 Microsoft Endpoint Configuration Managerイメージ。](images/configmgr-device-collections.png)

2. [デバイス コレクション] **を右クリックし** 、[デバイス コレクション **の作成] を選択します**。

    ![ウィザード 2 Microsoft Endpoint Configuration Managerイメージ。](images/configmgr-create-device-collection.png)

3. [名前]**と [制限****コレクション] を指定し、[** 次へ] を **選択します**。

    ![ウィザード 3 Microsoft Endpoint Configuration Managerのイメージ。](images/configmgr-limiting-collection.png)

4. [ルール **の追加] を選択** し、[ **クエリ ルール] を選択します**。

    ![ウィザード 4 Microsoft Endpoint Configuration Managerのイメージ。](images/configmgr-query-rule.png)

5. [直接 **メンバーシップ ウィザード** ] で **[次へ] をクリックし** 、[クエリ ステートメントの **編集] をクリックします**。

     ![ウィザード 5 Microsoft Endpoint Configuration Managerのイメージ。](images/configmgr-direct-membership.png)

6. [ **条件] を** 選択し、星のアイコンを選択します。

     ![ウィザード 6 Microsoft Endpoint Configuration Managerイメージ。](images/configmgr-criteria.png)
...
3. [パッケージ **のダウンロード] を選択します**。
    ...
    ![Microsoft Defender for Endpoint オンボーディング ウィザード 11 のイメージ。](images/mdatp-download-package.png)
...
6. [Microsoft **Defender ATP ポリシー] を右クリックし、[Microsoft** Defender ATP ポリシー **の作成] を選択します**。
    ...
    ![ウィザード 12 Microsoft Endpoint Configuration Managerイメージ。](images/configmgr-create-policy.png)

7. 名前と説明を入力し、[ **オン** ボーディング] が選択されている場合は、[次へ] を選択 **します**。

    ![ウィザード 13 Microsoft Endpoint Configuration Managerイメージ。](images/configmgr-policy-name.png)
...
12. 適切な利用統計情報 (標準または **迅速) を****選択し**、[次へ] を **クリックします**。
    ...
    ![構成設定 2 のイメージ。](images/configmgr-telemetry.png)
...
15. コンソールでMicrosoft Endpoint Configuration Manager作成した Defender for Endpoint ポリシーを右クリックし、[展開] を **選択します**。

     ![構成設定のイメージ 4.](images/configmgr-deploy.png)

16. 右側のパネルで、前に作成したコレクションを選択し **、[OK] をクリックします**。

    ![構成設定のイメージ 5。](images/configmgr-select-collection.png)
...
4. [インストール] Microsoft Monitoring Agent (MMA) をインストールします。
   ...
   - [4.5 .NET Framework (以降](https://www.microsoft.com/download/details.aspx?id=30653)**)** または [KB3154518 のいずれかをインストールします](https://support.microsoft.com/help/3154518/support-for-tls-system-default-versions-included-in-the-net-framework)。 両方を同じシステムにインストールしない。
...
2. [**スケジュールされたスキャン****]、[** スキャン設定]、[既定のアクション]、[リアルタイム保護]、[除外設定]、[詳細設定]、[脅威の上書き]、**クラウド保護サービス** とセキュリティ インテリジェンスの更新を選択し **、[OK] を選択します**。   
    ...
    ![次世代保護ウィンドウ 1 のイメージ。](images/1566ad81bae3d714cc9e0d47575a8cbd.png)
    ...
    ![次世代保護ウィンドウ 3 のイメージ。](images/36c7c2ed737f2f4b54918a4f20791d4b.png)

    ![次世代保護ウィンドウ 4 のイメージ。](images/a28afc02c1940d5220b233640364970c.png)

    ![次世代保護ウィンドウ 5 のイメージ。](images/5420a8790c550f39f189830775a6d4c9.png)

    ![次世代保護ウィンドウ 6 のイメージ。](images/33f08a38f2f4dd12a364f8eac95e8c6b.png)

    ![次世代保護ウィンドウ 7 のイメージ。](images/41b9a023bc96364062c2041a8f5c344e.png)

    ![次世代保護ウィンドウ 8 のイメージ。](images/945c9c5d66797037c3caeaa5c19f135c.png)

    ![次世代保護ウィンドウ 9 のイメージ。](images/3876ca687391bfc0ce215d221c683970.png)
...
1. [セキュリティ コンソールMicrosoft Endpoint Configuration Manager、Exploit Guard の [アセットとコンプライアンスの概要] Endpoint Protection Windows Defender **\> \> \> し、[Exploit Guard** ポリシーの作成]**を選択します**。
   ...
   ![コンソール 0 Microsoft Endpoint Configuration Managerイメージ。](images/728c10ef26042bbdbcd270b6343f1a8a.png)
...
4. 攻撃表面 **縮小ルール** レポートの [構成] タブをクリックします。 各デバイスの ASR ルール構成の概要と ASR ルールの状態が表示されます。
    ...
    ![攻撃表面の縮小ルール レポート 1 のスクリーンショット。](images/f91f406e6e0aae197a947d3b0e8b2d0d.png)

5. 各デバイスをクリックすると、ASR ルールの構成の詳細が表示されます。

    ![攻撃表面の縮小ルール レポート 2 のスクリーンショット。](images/24bfb16ed561cbb468bd8ce51130ca9d.png)
...
3. 設定を [監査] に **設定し、[次** へ] を **クリックします**。
    ...
    ![Configuration Manager2 System Centerスクリーンショット。](images/c039b2e05dba1ade6fb4512456380c9f.png)

4. [次へ] をクリックして、新しい Exploit Guard ポリシーを **確認します**。

    ![スクリーンショット Exploit Guard ポリシー1。](images/0a6536f2c4024c08709cac8fcf800060.png)
