...
5. [Symantec をアンインストールします](#uninstall-symantec)。

6. [Microsoft Defender for Endpoint が正しく動作するようにします](#make-sure-microsoft-defender-for-endpoint-is-working-correctly)。
...
|macOS:<p>11.3.1 (Big Sur)<p>10.15 (Catalina)<p>10.14 (Mojave)     |で DIY アプリをダウンロードして使用します [https://aka.ms/mdatpmacosdiy](https://aka.ms/mdatpmacosdiy) 。 <p>詳細については [、「Microsoft Defender for Endpoint on macOS」を参照してください](microsoft-defender-endpoint-mac.md)。        |
...
|PowerShell | 1. デバイスでWindows管理者としてWindows PowerShell開きます。 <p>2. コマンドレットを `Get-MpComputerStatus` 実行します。 <p>3. 結果の一覧で **、AMRunningMode: パッシブ** モードまたは **AMRunningMode: SxS パッシブ** モードのいずれかを探します。 |

> [!NOTE]
> 一部のバージョンWindows Defender ウイルス対策のMicrosoft Defender ウイルス対策の代わりに、Windows。
...
3. デバイスからシマンテックを削除します。 このヘルプが必要な場合は、Broadcom のドキュメントを参照してください。 Broadcom のリソースを次に示します。 
   ...
   - [Symantec Endpoint Protection](https://knowledge.broadcom.com/external/article/156148/uninstall-symantec-endpoint-protection.html)

   - Windows: [14 台のクライアントEndpoint Protection手動でアンインストールWindows](https://knowledge.broadcom.com/external/article?articleId=170040)
   ...
   - Linux デバイス: Linux 上のユーザーに関して[よく寄せられるEndpoint Protection質問](https://knowledge.broadcom.com/external/article?articleId=162054)

## <a name="make-sure-microsoft-defender-for-endpoint-is-working-correctly"></a>Microsoft Defender for Endpoint が正しく動作するようにする

Symantec をアンインストールしたので、次の手順では、Defender for Endpoint が正しく動作することを確認します。 Microsoft Defender for Endpoint デモ シナリオ サイト () をご覧ください [https://demo.wd.microsoft.com](https://demo.wd.microsoft.com) 。 少なくとも次を含む、そのページで 1 つ以上のデモ シナリオを試してみてください。
- クラウドによる保護
