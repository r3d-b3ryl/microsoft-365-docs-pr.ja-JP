...
フォームに入力して送信します。 すべての例外 (現在と過去) を表示するには、[脅威 [](tvm-remediation.md)**&** の脆弱性管理] メニューの [修復] ページに移動し、[例外] タブを選択します。例外を作成する方法の詳細について説明 [します](tvm-exception.md#create-an-exception)。
