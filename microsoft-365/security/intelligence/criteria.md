...
* **ランサムウェア：** ファイルを暗号化したり、デバイスの使用を妨げるその他の変更を加えたりするマルウェアの種類。 その後、デバイスを再度使用する前に、支払いまたはその他のアクションを実行する必要があることを示す身代金メモが表示されます。 [ランサムウェアに関する詳細情報を参照してください](/security/compass/human-operated-ransomware)。

* **不正セキュリティ ソフトウェア:** セキュリティ ソフトウェアのふりをするが、保護を提供しないマルウェア。 この種類のマルウェアは、通常、デバイスに存在しない脅威に関するアラートを表示します。 また、サービスの支払いを説得しようとします。

* **トロイ：** 無害に見えようとするマルウェアの種類。 ウイルスやウォームとは異なり、トロイの木馬自体は拡散しません。 代わりに、ユーザーにダウンロードとインストールを仕掛ける正当な外観を試みます。 インストールされると、トロイの木馬は、個人情報の盗み、他のマルウェアのダウンロード、攻撃者によるデバイスへのアクセス権の付与など、さまざまな悪意のあるアクティビティを実行します。
...
* **ワーム：** 他のデバイスに拡散するマルウェアの種類。 ウォームは、電子メール、インスタント メッセージング、ファイル共有プラットフォーム、ソーシャル ネットワーク、ネットワーク共有、リムーバブル ドライブを介して拡散できます。 高度なウォームは、ソフトウェアの脆弱性を利用して伝達します。
