...
> [!TIP]
> クエリパフォーマンスを向上させるには、ルールの実行頻度と一致する時間フィルターを設定します。 最も短い頻度は _24 時間ごと_に行われるため、過去1日のフィルター処理はすべての新しいデータを対象とします。
