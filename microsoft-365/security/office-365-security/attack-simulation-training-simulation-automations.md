...
- **ペイロード名**
- **テクニック**: 前のページで選択した手法ごとに 1 つのペイロードとして選択する必要があります。
...
[ランディング **ページ] ページ** で、ユーザーに表示されるトレーニング ランディング ページを構成します。 次の設定を使用できます。

- **Header**
- **Body**

既定値を受け入れるか、カスタマイズできます。
...
- **[1** つ以上のソーシャル エンジニアリング手法の選択] ページで [ドライブバイ **URL]** を選択した場合にのみ **[](#select-one-or-more-social-engineering-techniques)**、この設定を使用できます。 既定では、この設定はオンになっています ( ![ トグル オン アイコン ](../../media/scc-toggle-on.png) )。
