...
- **フィルターの判定**: 判定値は、 [スパム対策メッセージ ヘッダー](anti-spam-message-headers.md)で説明されているように、使用可能なフィッシングとスパム フィルターの判定に関連します。 使用可能な値については、次の表で説明します。
  ...
  |**ユーザー許可**<sup>\*</sup>|`SFV:SFE`|送信者がユーザーの差出人セーフ リストに含まれているため、メッセージはスパム フィルター処理をスキップしました。|
  |**ユーザー ブロック**<sup>\*\*</sup>|`SFV:BLK`|送信者がユーザーの [ブロックされた送信者] リストに含まれているため、メッセージはスパム フィルターによってブロックされました。|
  |**ZAP**|該当なし|[0 時間の自動消去 (ZAP)](zero-hour-auto-purge.md) は、配信されたメッセージを迷惑メール Email フォルダーまたは検疫に移動しました。 [スパム対策ポリシー](configure-your-spam-filter-policies.md)でアクションを構成します。|

  <sup>\*</sup> 許可されたメッセージがサービスによってブロックされている可能性があるため、スパム対策ポリシーを確認します。

  <sup>\*\*</sup> これらのメッセージは配信されずに検疫する必要があるため、スパム対策ポリシーを確認します。

- **メッセージの宛先**: ユーザーがメッセージ内のペイロード URL をクリックしなかった場合でも、受信者に配信されたメッセージ (受信トレイまたは迷惑メール Email フォルダー) を調査する必要があります。 検疫済みメッセージを検疫から削除することもできます。 詳細については、「 [EOP で検疫された電子メール メッセージ](quarantine-email-messages.md)」を参照してください。
  - **フォルダーの削除**
  - **削除**
