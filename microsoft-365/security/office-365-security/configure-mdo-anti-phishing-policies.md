...
5. **偽装 :**[ **編集]** をクリックして、ポリシー内の保護された送信者と保護された送信者ドメインを変更します。 これらの設定は、受信メッセージの差出人アドレスで特定の送信者が (個別に、またはドメインによって) 検索するポリシーの条件です。 詳細については、「Microsoft Defender for Office 365」の「フィッシング対策ポリシーの偽装[設定」を参照してください](set-up-anti-phishing-policies.md#impersonation-settings-in-anti-phishing-policies-in-microsoft-defender-for-office-365)。

   - **保護するユーザーを追加する**: 既定値は Off **トグル** ![ オフです ](../../media/scc-toggle-off.png) 。 オンにするには、トグルを [オン] にスライドし、表示される [ユーザーの追加 ![ ] ](../../media/scc-toggle-on.png) ボタンをクリックします。 
   ...
   - **保護するドメインを追加する**: 次の設定の一方または両方を構成します。
     ...
     - **所有するドメインを自動的に含める**: 既定値は **Off トグル** ![ オフです ](../../media/scc-toggle-off.png) 。 オンにするには、トグルを [オン] トグル **オンにスライド** ![ します ](../../media/scc-toggle-on.png) 。

       所有しているドメインを表示するには、[所有する **ドメインの表示] を選択します**。

     - **カスタム ドメインを含める**: 既定値は Off **トグル** ![ オフです ](../../media/scc-toggle-off.png) 。 オンにするには、トグルを **[** オン] にスライドし、[ドメインの追加] ボックスにドメイン名 (contoso.com など) を入力し ![ ](../../media/scc-toggle-on.png) 、Enterキーを押し、必要に応じて繰り返します。
   ...
   - **アクション**: [編集] **をクリックします。**
     ...
     - **偽装ユーザーによって** 電子メールが送信される場合: [保護するユーザーの追加] で指定した保護されたユーザーの 1 つが送信者であるメッセージに対して、次のいずれかのアクション **を構成します**。
     ...
     - **偽装ドメインから** 電子メールが送信される場合: [保護するドメインの追加] で指定した保護されたドメインの 1 つで送信者のドメインがあるメッセージに対して、次のいずれかのアクションを構成 **します**。
   ...
   - [ **偽装の安全ヒントを有効にする] をクリック** し、次の設定を構成します。
     ...
     すべてのヒントの既定値は Off **トグル オフ** ![ です ](../../media/scc-toggle-off.png) 。 オンに設定するには、トグルを [オン]に[スライドします](../../media/scc-toggle-on.png)。

     完了したら、**[保存]** をクリックします。

   - **メールボックス インテリジェンス**:

     - **メールボックス インテリジェンスを有効にする:** 既定値は **On Toggle** [On です](../../media/scc-toggle-on.png)。 オフにする場合は、トグルを Off トグル **オフにスライド** ![ します ](../../media/scc-toggle-off.png) 。
...
6. **ス** プーフィング : [編集] をクリックしてスプーフィング インテリジェンスをオンまたはオフにし、Outlook で認証されていない送信者 ID をオンまたはオフにし、ブロックされたスプーフィングされた送信者からのメッセージに適用するアクションを構成します。 これらの設定の詳細については、「フィッシング対策ポリシー [のスプーフィング設定」を参照してください](set-up-anti-phishing-policies.md#spoof-settings)。
   ...
   - **認証されていない送信者の設定**: 次の設定を構成できます。
     - 認証されていない送信者の疑問符 **(?)** 記号を有効にする: メッセージが SPF または DKIM チェックに合格しない場合に、メッセージが DMARC または複合認証に合格しない場合は、Outlook の [[](email-validation-and-authentication.md#composite-authentication)差出人] ボックスで送信者の写真に疑問符を追加します。 既定値は [**オン**] です。 オフにする場合は、トグルを Off トグル **オフにスライド** ![ します ](../../media/scc-toggle-off.png) 。
     - **"via"** タグを有効にする: [From] ボックスの電子メール アドレスが DKIM 署名または **MAIL FROM** アドレスのドメインと異なる場合は、via タグ (chris@contoso.com 経由で fabrikam.com) を追加します。 既定値は [**オン**] です。 オフにする場合は、トグルを Off トグル **オフにスライド** ![ します ](../../media/scc-toggle-off.png) 。
