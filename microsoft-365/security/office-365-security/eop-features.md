...
|---|---|
|**スパム対策保護**||
|受信スパム検出|詳細については、「スパム対策保護」[を参照Microsoft 365。](anti-spam-protection.md) <p> EOP がオンプレミスの Exchange メールボックスを保護するハイブリッド環境では、迷惑メール ルールがメッセージを迷惑メール フォルダーに移動できるよう、EOP スパム フィルターの評決を変換するために、オンプレミス Exchange でメール フロー ルール (トランスポート ルールとも呼ばれる) を構成する必要があります。 詳細については、「ハイブリッド環境の迷惑メール フォルダーにスパムを配信する [EOP の構成」を参照してください](/exchange/standalone-eop/configure-eop-spam-protection-hybrid)。|
...
|Web 上のOutlookまたはOutlook経由でスパムを管理する (以前は Outlook Web App)|管理者およびエンド ユーザーは、安全な送信者の一覧および受信拒否送信者の一覧を作成できます。 詳細については、「迷惑メールの[設定について」を参照Outlook。](configure-junk-email-settings-on-exo-mailboxes.md#about-junk-email-settings-in-outlook) <p> EOP を使用してオンプレミスのメールボックスを保護する場合は、ディレクトリ同期を使用して、これらの設定がサービスに確実に同期されます。 ディレクトリ同期の設定の詳細については、「スタンドアロン EOP でメール ユーザーを管理する」の「ディレクトリ同期を使用してメール ユーザーを管理する」 [を参照してください](/exchange/standalone-eop/manage-mail-users-in-eop)。|
...
|**メール フロー ルール**||
|ポリシー ベースのフィルタリングとアクション|カスタム ポリシーは、メール フロー Exchange (トランスポート ルールとも呼ばれる) に基づいて作成されます。 ドメイン、キーワード、ファイル名、ファイル タイプ、件名行、メッセージ本文、送信者、受信者、ヘッダー、IP アドレスによるフィルターが可能です。 詳細については、「メール フロー ルール (トランスポート[ルール)」を参照Exchange Online Protection。](mail-flow-rules-transport-rules-0.md)|
|テキスト パターンによるフィルター|メール フロー ルールでは、配列または正規表現を使用してテキストと一致できます。 1 つの文字列または複数の文字列の配列を使用して、アドレス、件名、本文、または添付ファイル名など、多くのメッセージ プロパティと一致させることもできます。 詳細については、「メール フロー ルール (トランスポート ルール[)」を参照Exchange Online Protection](mail-flow-rules-transport-rules-0.md)|
...
|送信者へのポリシー ルール通知の送信|メッセージを拒否し、配信不可レポート (NDR またはバウンス メッセージとも呼ばれる) を送信者に送信するには、[説明付きメッセージを拒否する] または [拡張状態コードを使用してメッセージを拒否する] アクションを使用します。 詳細については、「メール フロー ルール[のアクション」を参照Exchange Online。](/Exchange/security-and-compliance/mail-flow-rules/mail-flow-rule-actions)|
|メッセージのリダイレクトまたはコピー|メール フロー ルールでは、リダイレクト、Cc または BCC による受信者の追加、受信者の追加、その他のオプションの追加が可能です。 詳細については、「メール フロー ルール[のアクション」を参照Exchange Online。](/Exchange/security-and-compliance/mail-flow-rules/mail-flow-rule-actions)|
|複数のルール間でルールの優先度を調整する|Exchange 管理センターを使用して、ルールが処理される順序を変更します。|
|メッセージをフィルター処理し、メッセージのルーティングまたは属性を変更する|さまざまな条件に基づいてメッセージをフィルターしてから、一連のアクションを各メッセージに適用できます。 詳細については、「メール フロー ルール (トランスポート[ルール)」を参照Exchange Online Protection。](mail-flow-rules-transport-rules-0.md)|
|ルール別にメッセージのスパム信頼レベル (SCL) を変更します。|送信中のメッセージを検査し、選択した基準に基づいて Spam Confidence Level レベルを割り当てることができます。 詳細については、「メール フロー ルール [を使用してメッセージのスパム信頼レベル (SCL) を設定する」を参照してください](/exchange/security-and-compliance/mail-flow-rules/use-rules-to-set-scl)。|
|メッセージの添付ファイルの検査|添付ファイルの内容または添付ファイルの特性を調べて、調べた内容に基づいて実行するアクションを定義できます。 詳細については、「メール フロー ルール[を使用したメッセージの添付](/exchange/security-and-compliance/mail-flow-rules/inspect-message-attachments)ファイルの検査」を参照Exchange Online。|
|**管理**||
|Web ベースの管理|ほとんどの機能は、セキュリティ コンプライアンス [センター&管理されます](grant-access-to-the-security-and-compliance-center.md)。 <p> その他の機能では、管理センター (EAC) Exchange管理が必要です。 詳細については、「スタンドアロン EOP [Exchange管理センター Exchange Online](/exchange/exchange-admin-center)管理センター Exchange[を参照してください](/exchange/standalone-eop/exchange-admin-center-eop)。|
|ディレクトリ同期|ディレクトリ同期は、Azure Active Directory 同期ツール から利用できます。 詳細については、「スタンドアロン EOP でメール ユーザーを管理する」の「ディレクトリ同期を使用してメール ユーザーを管理する」 [セクションを参照してください](/exchange/standalone-eop/manage-mail-users-in-eop)。|
|ディレクトリ ベースのエッジ ブロック (DBEB)|DBEB 機能では、サービス ネットワーク境界で無効な受信者宛てのメッセージを拒否することができます。 DBEB を使用すると、管理者はメールが有効な受信者を Microsoft 365 に追加し、Microsoft 365 に存在しない電子メール アドレスに送信されるメッセージをMicrosoft 365。 DBEB の構成の詳細については、「Use Directory Based Edge Blocking to [reject messages to invalid recipients 」を参照してください](/exchange/mail-flow-best-practices/use-directory-based-edge-blocking)。|
|PowerShell|完全な EOP 機能は、スタンドアロンの EOP PowerShell で使用できます。 詳細については[、「PowerShell のExchange Online Protectionを参照してください](/powershell/exchange/exchange-online-protection-powershell)。|
|**レポート作成とログ記録**||
