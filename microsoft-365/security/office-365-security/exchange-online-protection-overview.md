...
EOP セキュリティ機能をセットアップする手順と、Microsoft Defender で使用するセキュリティと比較する手順については、「Office 365保護」を[参照してください](protect-against-threats.md)。 EOP 機能の推奨設定は、「[EOP および Microsoft Defender](recommended-settings-for-eop-and-office365.md) のセキュリティに関する推奨設定Office 365使用できます。
...
3. メッセージはポリシー フィルター処理を通じて続行され、作成したメール フロー ルール (トランスポート ルールとも呼ばれる) に対して評価されます。 たとえば、ルールは、特定の送信者からメッセージが届いたときに、マネージャーに通知を送信できます。
   ...
   サービス ライセンスを使用して CAL をExchange Enterpriseオンプレミス組織では、EOP でのデータ損失防止 [(DLP)](/exchange/security-and-compliance/data-loss-prevention/data-loss-prevention) チェックもこの時点で行います。
...
|監査レポート|[管理センターでのExchangeの監査](/exchange/security-and-compliance/exchange-auditing-reports/exchange-auditing-reports)|
...
|Office 365 Message Encryptionとして使用できる場合|詳細については、「[Office 365 での暗号化](../../compliance/encryption.md)」をご覧ください。|
|||
