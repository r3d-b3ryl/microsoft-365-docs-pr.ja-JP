...
- **オン: セーフ リンクは、ユーザーが電子メール内のリンクをクリックしたときに既知の悪意のあるリンクの一覧をチェックします。電子メール メッセージの** セーフ リンク スキャンをオンまたはオフにします。 推奨値は選択 (オン) で、次のアクションが実行されます。
  ...
  - **URL を書き換えないでください。SafeLinks API のみを使用してチェックを行います**。この設定がオン (オン) の場合、URL の折り返しは行われません。 安全なリンクは、URL クリック時に API を介して排他的に呼び出されます。これは、それをサポートする Outlook クライアントによって行われます。 推奨値が選択されます (オン)。
