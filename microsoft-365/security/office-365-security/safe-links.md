...
- **Microsoft Teams**: Teams の会話、グループ チャット、またはチャネル内のリンクに対する安全なリンク保護は、セーフ リンク ポリシーによって制御されます。
...
- **Office アプリ**: サポートされている Office デスクトップ、モバイル、および Web アプリの安全なリンク保護は、セーフ リンク ポリシーによって制御されます。

  Office アプリのセーフ リンク保護の詳細については、この記事の後半の [「Office アプリのセーフ リンク設定](#safe-links-settings-for-office-apps) 」セクションを参照してください。
...
- **安全なリンク ポリシーの設定**: これらの設定は、特定のポリシーに含まれているユーザーにのみ適用され、ポリシー間で設定が異なる場合があります。これらの設定には以下が含まれます:
  ...
  - [Office アプリのセーフ リンク設定](#safe-links-settings-for-office-apps)
...
|シナリオ|結果|
|---|---|
|Jean は、マーケティング部門のメンバーです。 Office アプリのセーフ リンク保護は、マーケティング部門のメンバーに適用されるセーフ リンク ポリシーで有効になります。 メール メッセージで [PowerPoint プレゼンテーション] を開き、[プレゼンテーションの URL] をクリックします。|Safe は安全なリンクによって保護されています。 <p> ジャンは、Office アプリのセーフ リンク保護が有効になっているセーフ リンク ポリシーに含まれています。 <p> Office アプリのセーフ リンク保護の要件の詳細については、この記事の後半の [「Office アプリのセーフ リンク設定](#safe-links-settings-for-office-apps) 」セクションを参照してください。|
|Chris の Microsoft 365 E5 組織には、安全なリンク ポリシーが構成されていません。 Chris は、悪意のある Web サイトへの URL を含む外部送信者から、最終的にクリックしたメールを受信します。|Chris はセーフ リンクによって保護されています。 <p> **組み込みの保護** プリセット セキュリティ ポリシーは、すべての受信者 (カスタムのセーフ リンク ポリシーまたは Standard または Strict プリセットのセキュリティ ポリシーで定義されていないユーザー) に安全なリンク保護を提供します。 詳しくは、「[EOP と Microsoft Defender for Office 365 の事前設定されたセキュリティ ポリシー](preset-security-policies.md)」を参照してください。|
|Pat の組織では、管理者が Pat を適用するセーフ リンク ポリシーを作成しましたが、Office アプリのセーフ リンク保護は無効になっています。 Pat が Word 文書を開き、ファイル内の URL をクリックします。|Pat は安全なリンクで保護されていません。 <p> Pat はアクティブなセーフ リンク ポリシーに含まれていますが、そのポリシーでは Office アプリのセーフ リンク保護が無効になっているため、保護を適用できません。|
...
## <a name="recipient-filters-in-safe-links-policies"></a>セーフ リンク ポリシーの受信者フィルター
...
安全なリンクは、受信メールをスキャンして、悪意のあるハイパーリンクを検出します。 スキャンされた URL は、Microsoft 標準 URL プレフィックス`https://nam01.safelinks.protection.outlook.com`を使用して書き換えまたは _ラップされます_。 リンクを書き換えた後、潜在的な悪意のあるコンテンツがないか分析します。
...
受信トレイ ルールまたは SMTP 転送による _自動_ 転送の場合、次のいずれかのステートメントに該当しない _限り_ 、最終的な受信者を対象とするメッセージで URL は書き換えられません。

- 受信者は、安全なリンクでも保護されます。
- URL は、以前の通信で既に書き換えられました。

セーフ リンク保護が有効になっている限り、URL が書き換えられたかどうかに関係なく、メッセージ配信の前に URL がスキャンされます。 サポートされているバージョンの Outlook (Outlook for Desktop バージョン 16.0.12513 以降) では、ラップされていない URL は、クリック時にクライアント側の API 呼び出しによってセーフ リンクに対してチェックされます。

メール メッセージに適用される安全なリンク ポリシーの設定については、次のリストを参照してください。

- **オン: セーフ リンクは、ユーザーが電子メール内のリンクをクリックしたときに既知の悪意のあるリンクの一覧をチェックします。電子メール メッセージの** セーフ リンク スキャンをオンまたはオフにします。 推奨値は選択 (オン) で、次のアクションが実行されます。
  - Windows の Outlook (C2R) では、セーフ リンク スキャンが有効になっています。
  ...
  次の設定は、電子メール メッセージのセーフ リンク スキャンが有効になっている場合にのみ使用できます。

  - **組織内で送信された電子メール メッセージに安全なリンクを適用する**: 同じExchange Online組織内の内部送信者と内部受信者の間で送信されたメッセージに対するセーフ リンク スキャンを有効または無効にします。 推奨値は選択 (オン) です。

  - **疑わしいリンクとファイルを指すリンクのリアルタイム URL スキャンを適用** する: ダウンロード可能なコンテンツを指す電子メール メッセージ内のリンクを含む、リンクのリアルタイム スキャンを有効にします。 推奨値は選択 (オン) です。
  ...
  - **URL を書き換えないでください。SafeLinks API のみを使用してチェックを行います**。この設定がオン (オン) の場合、URL の折り返しは行われません。 サポートされているバージョンの Outlook (Outlook for Desktop バージョン 16.0.12513 以降) では、URL クリック時に API を介して安全なリンクが排他的に呼び出されます。
...
3. 安全なリンクは、Web サイトを開く前に URL をすぐに確認します。
   ...
   - URL がダウンロード可能なファイルを指し示し、ファイルを **指す疑わしいリンクとリンクのリアルタイム URL スキャンを適用** する設定が、ユーザーに適用されるポリシーでオンになっている場合、ダウンロード可能なファイルがオンになります。

   - URL が安全であると判断された場合は、その Web サイトが開きます。

## <a name="safe-links-settings-for-microsoft-teams"></a>Microsoft Teams の安全なリンク設定

安全なリンク ポリシーで Microsoft Teams のセーフ リンク保護を有効または無効にします。 具体的には、[ **オン: 安全なリンク] を使用すると、ユーザーが Microsoft Teams でリンクをクリックしたときに、既知の悪意のあるリンクの一覧がチェック** されます。 推奨される値はオン (選択) です。
...
- [ユーザーが元の **URL にクリックして元の URL にアクセスできるようにする**] 設定を構成する方法に応じて、ユーザーは元の URL をクリックして元の URL に移動することを許可するか、許可されません (**スクリーンショットでは続行 (推奨されません)。** ユーザーが元の URL にクリックできないように、[ **ユーザーが元の URL までクリック** できるようにする] 設定を選択しないことをお勧めします。

リンクを送信したユーザーが、Teams 保護が有効になっているセーフ リンク ポリシーによって保護されていない場合、ユーザーはコンピューターまたはデバイス上の元の URL まで自由にクリックできます。
...
2. Microsoft 365 は、ユーザーの組織にMicrosoft Defender for Office 365が含まれていることと、Microsoft Teams の保護が有効になっているアクティブなセーフ リンク ポリシーにユーザーが含まれていることを確認します。

3. URL は、チャット、グループ チャット、チャネル、タブでユーザーのクリック時に検証されます。

## <a name="safe-links-settings-for-office-apps"></a>Office アプリのセーフ リンク設定

Office アプリの安全なリンク保護は、電子メール メッセージ内のリンクではなく、Office ドキュメント内のリンクをチェックします。 ただし、添付されている Office ドキュメントのリンクは、ドキュメントを開いた後にメール メッセージで確認できます。

セーフ リンク ポリシーで Office アプリのセーフ リンク保護を有効または無効にします。 具体的には、[ **オン: 安全なリンク] を使用すると、ユーザーが Microsoft Office アプリのリンクをクリックしたときに、既知の悪意のあるリンクの一覧がチェック** されます。 推奨される値はオン (選択) です。

Office アプリのセーフ リンク保護には、次のクライアント要件があります。
...
高いレベルでは、Office アプリの URL に対するセーフ リンク保護のしくみを次に示します。 サポートされている Office アプリについては、前のセクションで説明します。
...
> [!NOTE]
> 各セッションの開始時に、Office アプリの安全なリンクがユーザーが使用できることを確認するのに数秒かかる場合があります。

## <a name="click-protection-settings-in-safe-links-policies"></a>セーフ リンク ポリシーの [保護の設定] をクリックする

これらの設定は、電子メール、Teams、Office アプリのセーフ リンクに適用されます。

- **ユーザーのクリックを追跡する**: クリックされた URL のセーフ リンク クリック データの保存を有効または無効にします。 この設定はオンのままにすることをお勧めします。

  Office アプリのセーフ リンクでは、この設定はデスクトップ バージョンの Word、Excel、PowerPoint、Visio に適用されます。

  内部送信者と内部受信者の間で送信されるメール メッセージ内のリンクの URL クリック追跡は、現在サポートされていません。

  この設定を選択すると、次の設定を使用できます。

  - **ユーザーが元の URL までクリック** できるようにする: ユーザーが [警告ページ](#warning-pages-from-safe-links) をクリックして元の URL に移動できるかどうかを制御します。 推奨値が選択されていません (オフ)。

    Office アプリのセーフ リンクでは、この設定はデスクトップ バージョンの Word、Excel、PowerPoint、Visio の元の URL に適用されます。
...
複数のポリシーを作成した後、適用される順序を指定できます。 2つのポリシーが同じ優先順位を持つことはできません。最初のポリシーが適用されると、ポリシーの処理は停止します。 **組み込みの保護** ポリシーは常に最後に適用されます。 **Standard** および **Strict** の事前設定済みセキュリティ ポリシーに関連付けられているセーフ リンク ポリシーは、常にカスタムセーフ リンク ポリシーの前に適用されます。

優先順位と複数のポリシーの評価と適用方法の詳細については、「 [事前設定されたセキュリティ ポリシーとその他のポリシーの優先順位](preset-security-policies.md#order-of-precedence-for-preset-security-policies-and-other-policies) と [電子メール保護の順序と優先順位」を](how-policies-and-protections-are-combined.md)参照してください。
...
- 電子メール メッセージ
- Windows と Mac の Office アプリのドキュメント。
