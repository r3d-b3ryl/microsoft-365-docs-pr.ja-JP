...
|**メールボックスインテリジェンスを有効にする** <br/> |このポリシーのメールボックスインテリジェンスを有効または無効にします。 メールボックスインテリジェンスを有効にできるのは、クラウドベースのアカウント (つまり、Office 365 でメールボックスが完全にホストされているアカウント) のみです。 <br/> | メールボックスインテリジェンスは、組織内と組織外の両方からメールを送受信し、メールを受信するユーザーに基づいて構築されます。 <br/> |
...
> [!IMPORTANT]
> ![Image](../media/EnableMailboxIntelligenceBasedImpersonation.png)新しい重要な設定、separting * * メールボックスインテリジェンス "From" mailbox 知能 based impersonation protection "。 偽装は、プログラムを使用してメールボックスにアクセスし、スクリプト、API、サードパーティソフトウェア、またはメールボックスの下に悪意のあるコードを使用して、メールボックスにアクションを実行するときに発生します。
