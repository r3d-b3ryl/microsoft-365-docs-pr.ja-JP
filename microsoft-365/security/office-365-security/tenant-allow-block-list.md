...
> [!NOTE]
> [セーフ リンク ポリシーの [次の URL をブロックする] の一覧](safe-links.md#block-the-following-urls-list-for-safe-links)は非推奨になっています。 テナント許可/ブロック リスト で URL エントリのブロックを管理できるようになりました。 既存のエントリを "次の URL をブロックする" リストから移行し、テナント許可/ブロック リストの URL エントリをブロックします。 ブロックされた URL を含むメッセージは検疫されます。
...
- テナント許可/ブロック リストには、次の制限があります。
  - ドメイン&アドレスの 500 エントリ。
  - URL の 500 エントリ。
  - ファイル ハッシュの 500 エントリ。
  - スプーフィング (スプーフィングされた送信者) 用の 1024 エントリ。

- テナント許可/ブロック リストのエントリには、次の制限があります。
  - ファイル ハッシュの場合は 64 文字。
  - URL の場合は 250 文字。

- エントリの 99.99% は 30 分以内にアクティブにする必要があります。 30 分以内にアクティブでないエントリには、最大 24 時間かかることがあります。 
...
2. 目的のタブを選択します。 使用可能な列は、選択したタブによって異なります。
   ...
   - **ドメイン&アドレス**: **アクション** で結果をグループ化できます。
   - **スプーフィングされた送信者**: **アクション** または **スプーフィングの種類** で結果をグループ化できます。
...
## <a name="view-domains--addresses-file-or-url-entries-in-the-tenant-allowblock-list"></a>テナント許可/ブロック一覧でドメイン&アドレス、ファイル、または URL エントリを表示する

テナント許可/ブロック リストでブロック ドメイン&アドレス、ファイル、または URL エントリを表示するには、次の構文を使用します。
...
申請ポータルまたはテナント許可/ブロック リストのブロック エントリを使用して許可エントリを追加した後、エントリがアクティブになったら、すぐにエントリの動作を開始する必要があります。 エントリの 99.99% は 30 分以内にアクティブにする必要があります。 30 分以内にアクティブでないエントリには、最大 24 時間かかることがあります。
