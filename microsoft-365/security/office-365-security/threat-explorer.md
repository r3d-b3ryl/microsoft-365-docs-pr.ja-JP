...
- URL フィルター間の修正 (*URL* と *URL ドメインと URL ドメイン**とパス*): 更新プログラムは、URL/クリックの判定を含むメッセージの検索に影響します。 プロトコルに依存しない検索のサポートを有効にしたので、URL を使用 `http`せずに検索できます。 既定では、別の値が明示的に指定されていない限り、URL 検索は http にマップされます。 次に例を示します。
