...
> [!IMPORTANT]
> Office 365脅威インテリジェンスは、Microsoft Defender for Office 365保護機能と共にプラン 2 に追加されました。 詳細については、「プランと価格[Microsoft Defender for Office 365サービス](https://products.office.com/exchange/advance-threat-protection)の説明」をMicrosoft Defender for Office 365[してください](/office365/servicedescriptions/office-365-advanced-threat-protection-service-description)。
...
トラッカーは、プラン 2 で取得する多くの素晴[Microsoft Defender for Office 365ほんの一部です](office-365-ti.md)。 脅威トラッカーには、 [注目に値](#noteworthy-trackers)するトラッカー、 [トレンド](#trending-trackers) トラッカー、 [追跡](#tracked-queries)クエリ、保存済み [クエリが含まれます](#saved-queries)。

組織の脅威追跡ツール<https://security.microsoft.com>を表示して使用するには、Microsoft 365 Defender ポータルを開き、[電子メール &**の脅威****トラッカー]**\> に移動します。 脅威トラッカー ページに直接移動 **するには** 、 を使用します <https://security.microsoft.com/threattrackerv2>。
...
傾向トラッカー (以前はキャンペーンと呼ばばれ) は、過去 1 週間に組織のメールで受け取った新しい脅威を強調表示します。 Trending trackers ビューは、組織の環境に影響を与える電子メールの脅威を動的に評価Office 365します。 このビューには、テナント レベルのマルウェアの傾向が表示され、増加、フラット、または減少しているマルウェア ファミリが特定され、管理者は、どの脅威に対してさらに注意が必要かについてより詳細に把握できます。
...
注目すべき脅威として、添付ファイルによって検出された高度なマルウェアの[脅威セーフしています](safe-attachments.md)。 E5 のお客様Office 365 Enterprise、ユーザーが e5 Microsoft Defender for Office 365を使用していない場合は、[](defender-for-office-365.md)サブスクリプションに含まれている必要があります。 Defender for Office 365サービスでメール フローをフィルター処理する他のセキュリティ ツールがある場合でも、Office 365提供されます。 ただし、スパム対策とメール セーフ[リンク](safe-links.md)機能は、メインの電子メール セキュリティ ソリューションが電子メール セキュリティ ソリューションを使用している場合にOffice 365。

:::image type="content" source="../../media/policies.png" alt-text="ポータルMicrosoft Defender for Office 365のMicrosoft 365 Defender" lightbox="../../media/policies.png":::

今日の脅威が多い世界では、従来のマルウェア対策スキャンのみを実行すると、攻撃から十分に保護されていないという意味です。 今日のより高度な攻撃者は、一般的に利用可能なツールを使用して、従来の署名ベースのマルウェア対策エンジンでは認識されない新しい、難読化された、または遅延攻撃を作成します。 [セーフ添付ファイル] 機能は、電子メールの添付ファイルを受け取り、仮想環境で電子メールの添付ファイルを削除して、安全か悪意かを判断します。 このデトレーション プロセスは、仮想コンピューター環境内の各ファイルを開き、ファイルを開いた後に何が起こるかを監視します。 PDF、圧縮ファイル、または Office ドキュメントの場合、悪意のあるコードはファイル内で非表示にされ、被害者がコンピューターで開くとアクティブ化されます。 Defender for Office 365 機能は、電子メール フロー内のファイルを削除して分析することにより、動作、ファイル評価、および多くのヒューリスティック ルールに基づいてこれらの脅威を検出します。
...
- まだ行っていない場合は、リンクや添付ファイルなどの組織Microsoft Defender for Office 365の[](defender-for-office-365.md)詳細セーフ[構成](safe-links.md)セーフ[してください](safe-attachments.md)。
