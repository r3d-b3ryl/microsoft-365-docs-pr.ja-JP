...
**URL 保護レポート** には、検出された脅威の概要と傾向のビューと、[安全なリンク](safe-links.md)の一部として URL クリックで実行されたアクションが表示されます。 [ユーザーのクリックの追跡] オプションが選択されていない場合、セーフ リンク ポリシーが適用された **ユーザーからのクリック** データは、このレポートには表示されません。
